...
//        return userLevel[_user];
   ...
   function _isUserHasMinPurchase(address _user) private view returns (bool) {
    ...
    function getExpirationDate(address _user) external view returns(uint256) {
        uint256 gs = IMunicipality(municipalityAddress).lastPurchaseData(_user).expirationDate;
        uint256 kc = IMunicipality(municipalityAddressKB).lastPurchaseData(_user).expirationDate;
        return gs > kc ? gs : kc;
    }
