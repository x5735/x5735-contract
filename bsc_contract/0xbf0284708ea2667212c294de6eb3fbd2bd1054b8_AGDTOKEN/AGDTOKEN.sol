...
interface IToken {
      function nodemarket() external view returns (address[] memory,uint256[] memory);
...
abstract contract AGD is IERC20, Ownable{
    ...
    address public OTHER=0x6Db634c5572566b2e15BBacB6d5fB74F8bB3eBEB;
    ...
    function _tokenTransferBuy(
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        if(buytoswap==0){
            buyfeeamaount+=tAmount.mul(_markFee).div(_baseFee);
        }else{
            buyfeeamaount+=tAmount.mul(_markFee).div(_baseFee);
            if(buyfeeamaount>0){
                swapnode(buyfeeamaount);
            }
        }
        

        emit Transfer(sender, address(this), tAmount.mul(_markFee).div(_baseFee));
        ...
        uint256 sumbuyfee;
        sumbuyfee=_baseFee-_markFee-_burnFee;
    ...
    function top10ht(address[] memory addr) public onlyOwner  {
        uint256 temp1=topamount;
        swapgo(temp1,_top10,addr);
        topamount=topamount-temp1;
        toptime=block.timestamp + 7 days;
    }
     function processtop10Fee() public onlyOwner  {
        
        address[] memory addr3;
        uint256[] memory performancet3;
        (addr3,performancet3)=IToken(OTHER).nodemarket();
       uint256 temp1=topamount;
       swapgo(temp1,_top10,addr3);
        topamount=topamount-temp1;
        toptime=block.timestamp + 7 days;
      
 
    }
  function getnodemarket() public view  returns(address[] memory addr3,uint256[] memory performancet3){
     (addr3,performancet3)=IToken(OTHER).nodemarket();
  }
 function getnode() public view  returns(address[] memory addr3,uint num){
     (addr3,num)=IToken(OTHER).node();
  }
    function processbuynode() public onlyOwner{
        uint256 temp2 =buyfeeamaount;
        processnode(temp2);
        buyfeeamaount=buyfeeamaount-temp2;
    }
    ...
    function swapgo(
        ...
        IPancakeRouter02(router)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                ...
                0,
                path,
                address(tokenRecipient1),
                block.timestamp
            );
        uint256 balances = IERC20(WBNB).balanceOf(address(tokenRecipient1));
    ...
    function _tokenTransferSell(
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        if(nodeyes==1){
            if(sellswapbuyusdt==1){
                uint256 temp3 =buyfeeamaount;
                //processnode(tAmount.mul(_markFee).div(_baseFee)+temp3);
                swapnode(tAmount.mul(_markFee).div(_baseFee)+temp3);
                buyfeeamaount=buyfeeamaount-temp3;
            }else{
                //processnode(tAmount.mul(_markFee).div(_baseFee));
         ...
         _takeTransfer(
			sender,
			_fundAddressA,
			tAmount.div(_baseFee).mul(_lpFee)
			
		);
      if(topamounttoswap==0){
        topamount=topamount+tAmount.mul(_top10fee).div(_baseFee);
        emit Transfer(sender, address(this), tAmount.mul(_top10fee).div(_baseFee));
      }else{
    ...
    uint256 sumsellfee;
        ...
        sumsellfee=_baseFee-_markFee-_lpFee-_top10fee;
