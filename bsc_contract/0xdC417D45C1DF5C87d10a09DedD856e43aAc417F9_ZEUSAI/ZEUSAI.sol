...
contract ZEUSAI is ERC20, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            'ZEUSAI: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'ZEUSAI: incorrect fee percent');
        require(owner != address(0), 'ZEUSAI: incorrect owner address');
        require(mintToAddr != address(0), 'ZEUSAI: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'ZEUSAI: transfer from the zero address');
        require(recipient != address(0), 'ZEUSAI: transfer to the zero address');
