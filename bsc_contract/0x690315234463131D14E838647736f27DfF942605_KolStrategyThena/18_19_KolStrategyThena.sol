...
contract KolStrategyThena is Initializable, UUPSUpgradeable, FeeManager {
    ...
    function updateRoutes(ISolidlyRouter.route[] memory _outputToLp0Route, ISolidlyRouter.route[] memory _outputToLp1Route) external onlyOwner {
        if (outputToLp0Route.length>0) {
            require(_outputToLp0Route[0].from == dexNative
        ...
        if (outputToLp1Route.length>0) {
            require(_outputToLp1Route[0].from == dexNative
    ...
    function initParams(address _mainToken, address _pairToken, bool _stable, address _gauge,
        ...
        if (_outputToLp0Route.length>0) require(_outputToLp0Route[0].from == dexNative && _outputToLp0Route[_outputToLp0Route.length-1].to == lpToken0,"path0");
        for (uint i; i < _outputToLp0Route.length; ++i) {
            outputToLp0Route.push(_outputToLp0Route[i]);
        }

        if (_outputToLp1Route.length>0) require(_outputToLp1Route[0].from == dexNative && _outputToLp1Route[_outputToLp1Route.length-1].to == lpToken1,"path1");
        for (uint i; i < _outputToLp1Route.length; ++i) {
            outputToLp1Route.push(_outputToLp1Route[i]);
        }
