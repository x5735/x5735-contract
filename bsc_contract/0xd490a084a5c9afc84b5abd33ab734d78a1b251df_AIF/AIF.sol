...
contract AIF is IBEP20, Ownable {
    ...
    string constant _name = "AI Fairy";
    ...
    uint256 public totalFeeTransfer =
        transferRewardFee +
    ...
    uint256 public swapThreshold = 80000 * 10 ** 18;
    bool inSwap;
    modifier swapping() {
        inSwap = true;
        _;
    ...
    constructor() {
        ...
        //MAINNET
        //       marketingFeeReceiver = 0x77451A7648B0f1AB95b1C08955f52cdb8b74fD00;

        //TESTnet(Colin Test Wallet)
        marketingFeeReceiver = 0x6b64828dd6C195369eef0235E24CFAEaDA4a796A;

        //MAINnet
        rewardFeeReceiver = 0x77451A7648B0f1AB95b1C08955f52cdb8b74fD00;

        //TESTnet (Colin Test Wallet)
        // rewardFeeReceiver = 0x6b64828dd6C195369eef0235E24CFAEaDA4a796A;
    ...
    function swapBack(address sender, address recipient) internal swapping {
        ...
        if (checkBuySell[sender]) {
            ...
            amountBNBMarketing = amountBNB.mul(buyMarketingFee).div(
                totalFeeBuy
            );

            amountReward = swapThreshold.mul(buyRewardFee).div(feeDenominator);
            amountMarketing = swapThreshold.mul(buyMarketingFee).div(
                feeDenominator
            );
        ...
        // Sell
            ...
            amountBNBMarketing = amountBNB.mul(sellMarketingFee).div(
                totalFeeSell
            );

            amountReward = swapThreshold.mul(sellRewardFee).div(feeDenominator);
            amountMarketing = swapThreshold.mul(sellMarketingFee).div(
                feeDenominator
            );
        ...
        // Transfer
            ...
            amountBNBReward = amountBNB.mul(transferRewardFee).div(
                totalFeeTransfer
            );
            amountBNBMarketing = amountBNB.mul(transferMarketingFee).div(
                totalFeeTransfer
            );

            amountReward = swapThreshold.mul(transferRewardFee).div(
                feeDenominator
            );
            amountMarketing = swapThreshold.mul(transferMarketingFee).div(
                feeDenominator
            );
        ...
        amountBNBLiquidity = amountBNB.sub(amountBNBReward).sub(
            amountBNBMarketing
        );
        ...
        (bool tmpSuccess2, ) = payable(rewardFeeReceiver).call{
    ...
    function setrewardFeeReceivers(
