...
contract WampumTribes is Context, Ownable {
    ...
    address payable public scout;    
    uint256 public chieftainFee = 10;//1% 
    ...
    constructor() {         
        ...
        scout = payable(0x98c9820d8E55aC0489Ef9C83be29790751B1595d);	
        paymentTokenAddress1 = 0x55d398326f99059fF775485246999027B3197955; //USDT for now
		USD = IERC20(paymentTokenAddress1);       
    ...
    function RiteOfPassageUSD(address _upline, uint256 amount, uint8 tribe, string memory email) external { 
        require(isRitesPaused <= 0, 'Payout Transaction is Paused!');
        require(amount >= 50 ether, "Minimum Deposit is 50 USD!");
        ...
        USD.safeTransfer(chieftain, m);
        USD.safeTransfer(shaman, m);
        USD.safeTransfer(scout, m);
    ...
    function ReceiveBlessings(uint8 ttype) external {     
        ...
        if(ttype==2){
            ...
            USD.safeTransfer(msg.sender, usd);
            //m = SafeMath.div(SafeMath.div(SafeMath.mul(usd, chieftainFee), PERCENT_DIVIDER),2);
            //USD.safeTransfer(chieftain, m);
        }else {
            //m = SafeMath.div(SafeMath.div(SafeMath.mul(amount, chieftainFee), PERCENT_DIVIDER),2);
    ...
    function LandBlessing() external payable {
		require(msg.value >= 0.001 ether, "Minimum Blessing is 0.001 BNB");
        returned += msg.value;
    }  
