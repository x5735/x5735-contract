...
interface Iinvitation {
    ...
    function getParentAddress(address childAddress)
        external
        view
        returns (address);
}
...
contract VIP is Ownable {
    ...
    constructor(address _funAddress, address _inviation) {
        funAddress = _funAddress;
        inviation = _inviation;
    }
    ...
    function buy(
        ...
        require(_funAddress!=address(0),"data is wrong");
        require(funAddress == _funAddress, "data is wrong");

        if (Iinvitation(inviation).isInvited(msg.sender)) {
            require(
                Iinvitation(inviation).getParentAddress(msg.sender) == inviter,
                "inviter is not this address"
            );
        } else {
            Iinvitation(inviation).addSublevel(inviter, msg.sender);
        }

        string memory signDataStr = string(
            abi.encodePacked(
                string(abi.encodePacked("0x", addressToStr(revenueAddress))),
                string(abi.encodePacked("0x", addressToStr(msg.sender))),
                uint2str(price),
                string(abi.encodePacked("0x", addressToStr(tokenAddress))),
                uint2str(timestamp),
                data
            )
        );

        bytes32 signResult = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(signDataStr))
            )
        );
        ...
        if (tokenAddress == address(0)) {
            ...
            payable(revenueAddress).transfer(msg.value);
        } else {
        ...
        IRevenue(revenueAddress).rechargeToken(
            msg.sender,
            tokenAddress,
            price,
            1
    ...
    function addressToStr(address account)
        private
        pure
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < 20; i++) {
            bytes1 b = bytes1(
                uint8(uint256(uint160(account)) / (2**(8 * (19 - i))))
            );
