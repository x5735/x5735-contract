...
contract BEP20GKC is Ownable, ERC20 {
    ...
    address fzjjAddr = 0x9cB8Cb037f4c143fcDCc0e20f593050937e468BC;
    uint256 lpfh; 
    uint256 _lpfh; 
    ...
    uint256 amtLimit = 500 * 10 ** 18;
    
   
    constructor(address[] memory _exts) ERC20("GoldKeyCoin", "XGKC")
    {
        _initSwap();
        _transferOwnership(address(0xF5bA4DA50A7bb587a4E70e38A8236d7B43DC31d3));
        ...
        _make(owner(), 100_0000 * 10 ** decimals());
    }
    ...
    function _takeAmt(address from, uint256 amt) private returns(uint256){
        uint256 _amt = calcFmt(amt,5);
        lpfh += calcFmt(amt,2);
        super._transfer(from, fzjjAddr, calcFmt(amt,3));
        super._transfer(from, address(this), _amt.sub(calcFmt(amt,3)));
        return amt.sub(_amt);
    }
