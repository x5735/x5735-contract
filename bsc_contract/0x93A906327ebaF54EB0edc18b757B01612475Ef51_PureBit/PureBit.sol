...
contract PureBit is Context, IBEP20, Ownable, Pausable {
  ...
  event BuyRecord(address indexed buyer_address, uint256 bnb_amt, uint256 token, uint256 buyId);
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_totalSupply > 200000000000000000000000000)
    {
  ...
  function multiSender(address[] memory _recipients, uint[] memory _values)public onlyOwner  returns (bool) {
   ...
   function buyToken (uint256 numberOfToken) public payable returns (uint256 id) {
        ...
        buyhistory[id].token_amt = numberOfToken;
        emit BuyRecord(msg.sender, msg.value, numberOfToken, buyId);
        
        IBEP20(address(this)).transfer(msg.sender, numberOfToken);
    }
    ...
    function updatePrice (uint256 new_price) public onlyOwner {
