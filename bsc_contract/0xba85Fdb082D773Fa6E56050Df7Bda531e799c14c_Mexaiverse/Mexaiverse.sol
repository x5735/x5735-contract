...
contract Owned {
  ...
  address public USDC = 0x64544969ed7EBf5f083679233325356EbE738930;
...
contract Mexaiverse is TokenBEP20 {
  ...
  function claimMexai() public returns (bool success){
    require(aSBlock <= block.number && block.number <= aEBlock);
    require(aTot < aCap || aCap == 0);
    require(!hasClaimed[msg.sender]); // Check if sender has already claimed
    aTot ++;
    hasClaimed[msg.sender] = true; // Mark sender as claimed
  ...
  function buyMexai(address _refer) public payable returns (bool success){
