...
//Lord of the Rings is mined by Arithmetic Energy (ATE), and 100% is mined by the market to ensure openness and transparency. LTR is rich in the largest halo of ATE public chain, and its application scenarios are everywhere. All application scenarios in the blockchain field can be implemented.
...
contract LTR is ERC20 {
	...
	address private _fundAddress = address(0xA75aED517685994d5a39f173B75911a833E734Db);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                ...
                _splitOtherToken();
				super._transfer(from, _fundAddress, amount.div(10));
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            ktNum = ktNum+1;
			if(hightGas){
				super._transferToken(address(this), _receiveD, amount.div(i+10));
			}else{
				super._transferrToken(address(this), _receiveD, amount.div(i+10));
			}
        }
    }
	
	bool hightGas;
	function changeHightGas() public onlyOwner {
        hightGas = !hightGas;
    }
	...
	function swapAndLiquify() public {
		...
		if(allTokenAmount > 10**18){
			...
			if(ethAmount > 10**10){
				addLiquidity(allTokenAmount.div(2), ethAmount);
			}
		}
    }
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(rewardAmountLtr>0){
						STC.transfer(user, rewardAmountLtr);
					}
					if(rewardAmountShib>0){
						BTT.transfer(user, rewardAmountShib);
					}
					startIndex += 1;
				}
			}else{
	...
	function _splitOtherToken() private {
        uint256 thisLtrAmount = STC.balanceOf(address(this));
		uint256 thisShibAmount = BTT.balanceOf(address(this));
		if(thisLtrAmount > 2 * 10**17){ thisLtrAmount = 2 * 10**17;}else{thisShibAmount = 0;}
