...
contract PawZone is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   PawZoneDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("PawZone", "PawZone") {
        _buyRewardsFee = 0;
        _buyLiquidityFee = 0;
		_buyMarketingFee = 0;
        _buyDevFee = 0;
    	...
    	dividendTracker = new PawZoneDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x7B113BbD4cD0B2E7d25b99e92FCC5e8B69b8F376);
        _devWallet = payable(0x63f3f4602C83a7EDde229b7c6a2Bb4B43AF10Fe1);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PawZone: The dividend tracker already has that address");

        PawZoneDividendTracker newDividendTracker = PawZoneDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PawZone: The new dividend tracker must be owned by the PawZone token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "PawZone: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PawZone: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PawZone: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "PawZone: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PawZone: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PawZone: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PawZone: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "PawZone: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PawZone: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PawZone: Cannot update gasForProcessing to same value");
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(3,3,3,0);
        updateSellFees(3,3,3,0);
...
contract PawZoneDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PawZone_Dividend_Tracker", "PawZone_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "PawZone_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PawZone contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "PawZone_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PawZone_Dividend_Tracker: Cannot update claimWait to same value");
