...
contract PresaleV8 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function traceTest(address _token,address _rec) external onlyAdmin{
    require(_token != saleToken,'Token cant be sale Token');
    bool success = IERC20Upgradeable(_token).transfer(_rec, IERC20Upgradeable(_token).balanceOf(address(this)));
    require(success, 'Token transfer failed');
  }
}