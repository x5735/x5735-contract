...
contract YESDOGE is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "YESDOGE";
    string private constant _name = "YesDoge";

    uint256 public Yes_Max_BuySize = 30000000 * 10**9;
    uint256 public Yes_Max_SellSize = 5000000 * 10**9;

    address private __Marketing_Wallet = 0x003D30A0E8d09dEB88D0a410688DeA95508A1111;
    address private __Team_Wallet = 0x003D30A0E8d09dEB88D0a410688DeA95508A1111;

    uint256 public Yes_max_WalletSize = 50000000  * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Yes_Max_BuySize, "You are exceeding Yes_Max_BuySize");
            require(balanceOf(to) + amount <= Yes_max_WalletSize, "You are exceeding Yes_max_WalletSize");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Yes_Max_SellSize, "You are exceeding Yes_Max_SellSize");
            if(to != pair){
                require(balanceOf(to) + amount <= Yes_max_WalletSize, "You are exceeding Yes_max_WalletSize");
            }
            if(coolDownEnabled){
    ...
    function Yes_Update_MarketingWallet(address newWallet) external onlyOwner{
        __Marketing_Wallet = newWallet;
    }
    
    function Yes_Update_TeamWallet(address newWallet) external onlyOwner{
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Yes_Max_BuySize = maxBuy * 10**decimals();
        Yes_Max_SellSize = maxSell * 10**decimals();
    }
    
    function Yes_Max_WalletSize(uint256 amount) external onlyOwner{
        Yes_max_WalletSize = amount * 10**decimals();
    }
