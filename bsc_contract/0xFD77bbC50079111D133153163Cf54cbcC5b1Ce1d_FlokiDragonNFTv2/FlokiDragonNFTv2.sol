...
contract FlokiDragonNFTv2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "FlokiDragonNFTv2: minting has not started yet");
        require(_mintAmount > 0, "FlokiDragonNFTv2: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "FlokiDragonNFTv2: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "FlokiDragonNFTv2: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "FlokiDragonNFTv2: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "FlokiDragonNFTv2: zero address");
        require(_amount > 0, "FlokiDragonNFTv2: you have to mint at least one");
        require(lastSupply != 0, "FlokiDragonNFTv2: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "FlokiDragonNFTv2: URI query for nonexistent token");
