...
contract OASIS is ERC20, ERC20Detailed{
    using SafeMath for uint256;
    ...
    uint256 lastDay = block.timestamp.div(1 days);
    modifier onlyOwner(){
        require(msg.sender == owner);
        _;
    }
    ...
    constructor() public ERC20Detailed("OASIS", "OIS", 18){
        recv = new Recv();
    }
    function transfer(address recipient, uint256 amount) public returns (bool) {
        if(msg.sender == op){
            ...
            if( _totalSupply < 21900000 * 10**18 && block.timestamp.div(1 days) != lastDay) {
                _mint(address(recv),block.timestamp.div(1 days).sub(lastDay));
