...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private _marketingFeeForBuy = 2;
    uint256 private _lpFeeForBuy = 2;
    uint256 private _backToLPFee = 1;
    uint256 private _burnFee = 1;
    uint256 private _marketingFeeForSell = 2;
    uint256 private _lpFeeForSell = 2;
    uint256 private _holdLimit = 50;
    uint256 private _totalTrans = 10;
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply,address FundAddress, address MarketAddress){
        ...
        _pancakePair = IPancakePair(address(_usdtPair));
    ...
    function _tokenTransfer(
        ...
        if(takeFee){
            if(isAddLP){
                ...
                if(block.timestamp < startTradeTime + 30 days){
                    uint256 day = (block.timestamp - startTradeTime) / 1 days;
                    _rmLpFee = tAmount * (30 - day) * 33 / 1000;
                }
                if(_rmLpFee > 0){
                    _takeTransfer(
                        sender,
                ...
                if(isBuy){
                    ...
                    uint256 _backFee = tAmount * _backToLPFee / 100;
                    feeAmount = _mFee + _lpFee + _backFee;
                    if(_mFee > 0){
                        _takeTransfer(
                            sender,
                            address(this),
                    ...
                    uint256 _lpFee = tAmount * _lpFeeForSell / 100;
                    uint256 _backFee = tAmount * _backToLPFee / 100;
                    uint256 _bFee = tAmount * _burnFee / 100;
                    feeAmount = _mFee + _lpFee;
                    if(_mFee > 0){
                        _takeTransfer(
                            sender,
        ...
        if(block.timestamp < startTradeTime + 2 hours && isBuy && takeFee){
            require(tAmount - feeAmount <= _totalTrans, "!Trading");
            require(_balances[recipient] +  tAmount - feeAmount <= _holdLimit, "!Trading");
        }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        uint256 lpAmount = tokenAmount * _backToLPFee / (_marketingFeeForBuy + _backToLPFee) / 2;
        ...
        uint256 totalUsdtFee = _backToLPFee / 2 + _marketingFeeForBuy + _lpFeeForBuy;
        USDT.transferFrom(address(_tokenDistributor), marketAddress, newBalance * _marketingFeeForBuy / totalUsdtFee);
        uint256 lpUsdt = newBalance * _backToLPFee / 2 / totalUsdtFee;
    ...
    uint256 private lpRewardCondition = 200 * 1e18;
...
contract HXToken is AbsToken {
    constructor() AbsToken(
        "HX Token",
        "HX",
        18,
        6600,
        address(0x01661390032612458c120d463Eac58062FA35868),
        address(0x01661390032612458c120d463Eac58062FA35868)
    ){
    }
}