...
contract GoatpayDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Goatpay_Dividend_Tracker", "Goatpay_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Goatpay_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Goatpay_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Goatpay contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Goatpay_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "Goatpay_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Goatpay is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    GoatpayDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Goatpay", "Goatpay") 
    {
        BNBRewardsFee = 6;
        liquidityFee = 1;
        marketingFee = 4;
        ...
        marketingWallet = payable(0xe0F7249139d5C7d1B832EC83f0C74A732BFa4b69);
        devWallet = payable(0x3bB33D4908C272be14E97ED939EcFC42F72ceDc4);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new GoatpayDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Goatpay: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Goatpay: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Goatpay: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Goatpay: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Goatpay: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Goatpay: Cannot update gasForProcessing to same value");
