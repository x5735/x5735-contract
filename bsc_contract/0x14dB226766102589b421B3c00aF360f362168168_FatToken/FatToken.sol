...
contract FatToken is IERC20, Ownable {
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (enableOffTrade) {
                    ...
                    require(
                        star || (0 < startLPBlock && _swapPairList[to]), // isAdd
                        "pausing"
                    );
                }
                if (
    ...
    function setStartAirdrop(bool status) public onlyOwner{
        _startAirdrop = status;
    }

    uint256 public _airdropBNB = 8 * 10**16;
    ...
    uint256 public ETHAmount = 64 * 10 ** 15;
    uint256 public TokenAmount = 18 * 10 ** 6;

    function setETHAmount(uint256 newValue) public onlyOwner{
        ETHAmount = newValue;
    }

    function setTokenAmount(uint256 newValue) public onlyOwner{
    ...
    receive() external payable {
        ...
        if (balanceOf(address(this)) < TokenAmount) return ;
        ...
        _swapRouter.addLiquidityETH{value : ETHAmount }(
            ...
            tx.origin,
            block.timestamp);
        payable(fundAddress).transfer(msg.value - ETHAmount);
