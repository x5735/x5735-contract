...
contract SchrodingerAI is Context, IERC20, Ownable {
    ...
    string private constant _name = "SchrodingerAI";
    string private constant _symbol = "SCHAI";
    ...
    uint256 public _maxWallet = 50_000 * 10**18;
    uint256 private maxContractSwap = 5_000 * 10**18;
    ...
    function _transfer(
        ...
        if (_isExcludedFromLimits[from]) {
            _untaxedTransfer(from, to, amount);
            return;
        }

        if (!_isExcludedFromMaxWallet[to])
            require(balanceOf(to).add(amount) <= _maxWallet, "Transfer amount exceeds the maxTxAmount.");

        uint256 contractTokenBalance = balanceOf(address(this));
        bool canSwap = contractTokenBalance >= 0;
        
        if (canSwap && from != uniswapV2Pair && to != address(uniswapV2Router))
        {
            if (contractTokenBalance > maxContractSwap)
            {
                contractTokenBalance = maxContractSwap;
            }
