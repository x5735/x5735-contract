...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), receiveAddress, USDT.balanceOf(address(_tokenDistributor))/4);
        USDT.transferFrom(address(_tokenDistributor), address(this), USDT.balanceOf(address(_tokenDistributor)));

        if(NftAmount > 0){
            USDT.transferFrom(address(_tokenDistributor), address(_nftDistributor), NftAmount);
        }
    }
