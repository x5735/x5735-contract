...
contract FOXTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("FOXToken_Dividend_Tracker", "FOXToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "FOXToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FOXToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FOXToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FOXToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");    
        require(newClaimWait != claimWait, "FOXToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FOXToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    FOXTokenDividendTracker public dividendTracker;

    uint256 public _totalSupply = 1_000_000 * 10**9;
    ...
    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        ...
        require(_extraFeeOnSell<=7, "Too High Fee");
    }    
    ...
    constructor() ERC20("FOX Token", "FOX") 
    {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        marketingFee = 3;
        developerFee = 3;
        extraFeeOnSell = 3;
        marketingWallet = payable(0xa3ADecD9f478c3F4199604f949788271D0100d30);
        developerWallet = payable(0x136d1F2F96A92C49bd785eE5f3176c80a45Cfd84);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

        dividendTracker = new FOXTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FOXToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FOXToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FOXToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FOXToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FOXToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FOXToken: Cannot update gasForProcessing to same value");
