...
contract  worRg { 
       ...
       event BindingAddressCommunity(address indexed addr, string  code, string  name);
       event ReceiveAward(address indexed addr,  string  name ,uint num);
       event RgWor(address indexed addr,  string  name ,uint num);
       ...
       function getCommunityAdmin (string memory _code) public view returns(communityLp memory){
        ...
        function receiveAward() public {
                ...
                for (uint i = 0; i < u.length; i ++) {
                    ...
                    uint c =  day.div(day300) + daycount;
        ...
        function rgWor(uint _value) public  {
          ...
          uint usdtNum =  _value.mul(price);
          require(IPancakePair(USDT).balanceOf(msg.sender) >= usdtNum,"USDT balance deficiency");
          require(IBEP20(WOR).balanceOf(address(this)) >= _value,"underreward");
        ...
        //  IBEP20(USDT).transferFrom(msg.sender, address(this), usdtNum);
        ...
        function _utilCompareInternal(string memory a, string memory b)  public pure returns (bool) {
