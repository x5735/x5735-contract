...
contract MultiSig {
    ...
    function fundContractTokens(
        IERC20 _token,
        uint256 _amount
    ) external ownerExists(msg.sender) {
...
contract HorseaFiananceMutisign is Factory, Ownable {
    struct multiSig {
        address inMultiSigs;
        uint256 timeStamp;
    }
    ...
    function getInMultiSigs(
        address _addr
    ) public view returns (multiSig[] memory) {
    ...
    function createMultiSig(
        ...
        if (_subscription == 1) {
            require(_owners.length <= _sub1Owners, "Max owners reach");
        } else if (_subscription == 2) {
            require(_owners.length <= _sub2Owners, "Max owners reach");
        } else {
            require(_owners.length <= _sub3Owners, "Max owners reach");
        }
        ...
        if (_subscription == 1)
            wallet = new MultiSig(_owners, _required, s1TokenAmount);
        else if (_subscription == 2)
            wallet = new MultiSig(_owners, _required, s2TokenAmount);
        else wallet = new MultiSig(_owners, _required, s3TokenAmount);
