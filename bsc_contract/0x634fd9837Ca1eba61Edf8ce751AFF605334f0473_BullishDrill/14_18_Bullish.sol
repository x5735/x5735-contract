...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	{
		uint256 boosted_amount_e6 = _get_boosted_user_amount(_pool_id, _address_user, _user.staked_amount);
		uint256 pending = (boosted_amount_e6 * _pool.accu_reward_amount_per_share_e12) / 1e12;
		return pending;
	}
	...
	function get_pending_reward_amount(uint256 _pool_id) external view returns(uint256)
	{
		...
		uint256 boosted_amount_e6 = _get_boosted_user_amount(_pool_id, address_user, user.staked_amount);
		uint256 pending = (boosted_amount_e6 * new_pool_accu_e12) / 1e12;
		return pending;
	}
