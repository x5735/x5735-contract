...
interface events {
   /**
    ...
    event rewardTokenDistribution(address indexed seller, address indexed buyer, uint256 reward);
    event rewardDistribution(address user, uint256 amount);
}
...
contract Storage is events, Ownable {
    ...
    mapping(address => uint256) private userpRGPReward;
    ...
    function settleRewards(address _seller, address _buyer) private {
        if(rewardPRGP != 0 
            && IERC20(rewardToken).balanceOf(address(this)) > rewardPRGP
            && rewardToken != address(0)
        ) {
            userpRGPReward[_seller] = userpRGPReward[_seller] + rewardPRGP;
            userpRGPReward[_buyer] = userpRGPReward[_buyer] + rewardPRGP;
            emit rewardTokenDistribution(_seller, _buyer, rewardPRGP);
        }
    }

    function withdrawReward() external {
        require(userpRGPReward[msg.sender] > 0, "No Reward");
        uint256 userReward = userpRGPReward[msg.sender];
        userpRGPReward[msg.sender] = 0;
        IERC20(rewardToken).transfer(msg.sender, userReward);
        emit rewardDistribution(msg.sender, rewardPRGP);
    }
    ...
    function _transferAndEmit(uint256 productID, address to, uint256 remnant,uint256 subGas) internal {
        ...
        address __seller = allProducts[productID].seller;
        emit Buy(to, __seller, s_token, subGas, productID, block.timestamp);
        settleRewards(__seller, allProducts[productID].buyer);
    }
    ...
    function _tip(uint256 productID, address who) internal {
        ...
        // if (block.timestamp > uint256(raisedDispute[productID].votesEnded)) revert Be_Patient();
    ...
    function _l(uint256 productID, uint256 amountShared, uint256 fee,address token, address who) internal {       
        ...
        allProducts[productID].endSales = uint128(block.timestamp);
        settleRewards(allProducts[productID].seller, allProducts[productID].buyer);
    ...
    function setPRGPRewardsAmount(uint256 _rewardForEach) external onlyOwner {
        rewardPRGP = _rewardForEach;
    }
