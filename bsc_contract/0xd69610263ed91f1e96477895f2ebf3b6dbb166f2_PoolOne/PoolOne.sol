...
contract PoolOne is Ownable {
   ...
   */
    ...
    function allowStaking(bool _status) public onlyOwner returns(bool){
        require(tokenAddress != address(0) && feeADDRESS != address(0), "Neccessary addresses are not yet configured");
    ...
    function stake(uint amountToStake) public {
        ...
        require(Token(tokenAddress).balanceOf(msg.sender) >= amountToStake, "Insufficient token balance");
        require(Token(tokenAddress).allowance(msg.sender, address(this)) >= amountToStake, "Insufficient Token Allowance");
        require(Token(tokenAddress).transferFrom(msg.sender, address(this), amountToStake), "Failed transaction");
        ...
        require(Token(tokenAddress).transfer(feeADDRESS, fee), "Could not transfer tax fee.");
    ...
    function unstake(uint amountToWithdraw) public {
        ...
        require(Token(tokenAddress).transfer(admin, fee), "Could not transfer unstaking fee.");
