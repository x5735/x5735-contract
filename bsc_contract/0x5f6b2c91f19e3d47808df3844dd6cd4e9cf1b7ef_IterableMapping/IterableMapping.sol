...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function updateMinimumDividendForAutoClaim(
        ...
        require(
            _newMinimumDividend >= 1 * 10 ** 16 &&
                _newMinimumDividend <= 50 * 10 ** 18
        );
...
contract URANUSPAY is ERC20, Ownable {
    using SafeERC20 for IERC20;

    uint256 public liquifyTaxBuy;
    uint256 public marketingTaxBuy;
    uint256 public reflectionTaxBuy;

    uint256 public liquifyTaxSell;
    ...
    constructor() ERC20("URANUSPAY", "URP") {
        _mint(owner(), 1_000_000_000_000 * (10 ** 18));

        liquifyTaxBuy = 100;
        marketingTaxBuy = 200;
        reflectionTaxBuy = 500;

        liquifyTaxSell = 100;
        marketingTaxSell = 200;
        reflectionTaxSell = 500;
        buybackTaxSell = 500;

        denominator = 10_000;

        marketingWallet = 0xbCc9375A2e481e4914FD4D81e953b02f05051fD4;
        ...
        IERC20(getBUSDAddress()).safeApprove(
    ...
    function getBUSDAddress() public view returns (address) {
        if (block.chainid == 56) {
            ...
            revert("Cannot found busd on this network");
        }
    }
    ...
    function setBuyTax(
        uint256 _liquifyTaxBuy,
        uint256 _marketingTaxBuy,
        uint256 _reflectionTaxBuy
    ) external onlyOwner {
        require(
            liquifyTaxBuy != _liquifyTaxBuy ||
        ...
        emit UpdateBuyTax(_liquifyTaxBuy, _marketingTaxBuy, _reflectionTaxBuy);
    }

    function setSellTax(
        uint256 _liquifyTaxSell,
        uint256 _marketingTaxSell,
        uint256 _reflectionTaxSell,
        uint256 _buybackTaxSell
    ) external onlyOwner {
        require(
            liquifyTaxSell != _liquifyTaxSell ||
        ...
        liquifyTaxBuy = _liquifyTaxSell;
        marketingTaxBuy = _marketingTaxSell;
        reflectionTaxBuy = _reflectionTaxSell;
        buybackTaxSell = _buybackTaxSell;
    ...
    function buybackAndBurn(uint256 amount) external onlyOwner {
        IERC20 BUSDToken = IERC20(getBUSDAddress());
        uint256 BUSDBalance = BUSDToken.balanceOf(address(this));

        require(
            amount <= BUSDBalance,
            "BUSD Balance not enough in contract to buy back"
        );

        address[] memory path = new address[](2);
        path[0] = getBUSDAddress();
        path[1] = address(this);
        ...
        emit TriggerBuybackAndBurn(amount);
    }

    function claimStuckTokenFromSwapReceiver(address token) external onlyOwner {
        swapReceiver.claimStuckTokens(token);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                fees =
                    (amount *
                        (liquifyTaxBuy + marketingTaxBuy + reflectionTaxBuy)) /
                    denominator;
    ...
    function swapBack() internal inSwap {
        ...
        uint256 totalTax = liquifyTaxBuy +
            marketingTaxBuy +
            reflectionTaxBuy +
            liquifyTaxSell +
            marketingTaxSell +
        ...
        uint256 liquifyToken = ((contractTokenBalance *
            (liquifyTaxBuy + liquifyTaxSell)) / totalTax) / 2;
        ...
        swapReceiver.claimStuckTokens(getBUSDAddress());

        uint256 newBalance = BUSDToken.balanceOf(address(this)) -
            contractBUSDBalance;

        uint256 liquifyBUSD = ((newBalance * (liquifyTaxBuy + liquifyTaxSell)) /
            totalTax) / 2;
        uint256 marketingBUSD = (newBalance *
            (marketingTaxBuy + marketingTaxSell)) / totalTax;
        uint256 reflectionBUSD = (newBalance *
            (reflectionTaxBuy + reflectionTaxSell)) / totalTax;

        if (liquifyToken > 0 && liquifyBUSD > 0) {
        ...
        if (marketingBUSD > 0) {
            BUSDToken.safeTransfer(marketingWallet, marketingBUSD);
        }

        if (reflectionBUSD > 0) {
            BUSDToken.safeTransfer(address(dividendTracker), reflectionBUSD);
            dividendTracker.distributeDividends(reflectionBUSD);
            emit SendDividends(reflectionBUSD);
        }
    }
