...
contract TokenV2 is Ownable, AccessControl, ERC20Burnable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        for (uint256 i=0; i<plugins.length; i++) {
            amount = plugins[i].execute(sender, recipient, amount);
        }
        ...
        if (fullAmount - amount > 0) {
            emit Transfer(sender, address(this), fullAmount - amount);
            _afterTokenTransfer(sender, address(this), fullAmount - amount);
        }
    }
