...
contract BPCoin is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0x37DF92d3541500c75Cb62a6061317d2D20329bFE;

    string constant _name = "BitPocket Coin";
    ...
    function burnFrom(address from, uint256 amount ) public returns (bool success) {
        require( balanceOf(from) >= amount, "Not enough tokens in wallet");
        if(_allowances[from][msg.sender] != uint256(-1)){
                _allowances[from][msg.sender] = _allowances[from][msg.sender].sub(amount, "Insufficient Allowance");
        }
    ...
    function mintToken(address target, uint256 amount ) external onlyOwner  {
