...
abstract contract AGD2 is   Ownable {
    ...
    function cpbind(uint start,uint end) public onlyOwner{
        address fatheraddr;
        address toaddr;
         for (uint n = start; n < end; n++) {
            if (_cptoken.viplist(n) == address(0)) {
            ...
            uint256 len = _cptoken._binders(_cptoken.viplist(n)).length;
           
            if(len>0){
                fatheraddr=_cptoken.viplist(n);
                
                for(uint p=0;p<len;p++){
                    toaddr=_cptoken._binders(_cptoken.viplist(n))[p];
    ...
    function getothercount() public view  returns(uint256 count){
        count==_cptoken.vipcount();
    }
    function getotherbind(address account) public view  returns(address[] memory addr,uint256 count){
        count = _cptoken._binders(account).length;
        if(count>0){
            addr=_cptoken._binders(account);
        }
    }
    ...
    //  function cp() public onlyOwner{
    //     for (uint n = 0; n < IERC20(_cptoken).vipcount(); n++) {
    //         if (IERC20(_cptoken).viplist(n) == address(0)) {
    //             break;
    //         }
            
    //         inviter[IERC20(_cptoken).viplist(n)] = IERC20(_cptoken).inviter(IERC20(_cptoken).viplist(n));
    //         user[IERC20(_cptoken).viplist(n)]=IERC20(_cptoken).getuser(IERC20(_cptoken).viplist(n));
    //         viplist[n]=IERC20(_cptoken).viplist(n);
    //         if(n==0){
    //             address fi=IERC20(_cptoken).inviter(IERC20(_cptoken).viplist(n));
    //             user[fi]=IERC20(_cptoken).getuser(fi);
    //         }
            

    //     }
    //     vipcount=IERC20(_cptoken).vipcount();
    // }
    ...
    function ztjiang(uint256 agd) private{
        ...
        for (uint256 i = 0; i < maxzt;i++) {
            ...
            if(agd>0){
                uint256 curTAmount = agd;
                
                
                uint256 agdtousdtamount=curTAmount*agdpri2/10**18;
    ...
    function teamjiang(uint256 ltj) private{
        ...
        for (uint256 i = 0; i < maxteam;i++) {
            ...
            if(rate>0){
                ...
                uint256 agdtousdtamount=curTAmount*agdpri23/10**18;
                if(user[cur].edusum>agdtousdtamount){
                    user[cur].edusum=user[cur].edusum-agdtousdtamount;
                    user[cur].dongtai+=curTAmount;
                }else{
    ...
    // function pushzt2(address addr) public  onlyOwner {
          
    //     for (uint n = 0; n < vipcount; n++) {
             
    //         if(inviter[viplist[n]]==addr){
                 
    //            pushzt3(viplist[n],addr);
    //         }
   ...
   require(tx.origin == msg.sender, "CAN NOT");    
    ...
    function getprice() public view returns (uint256 _price) {
        if(agdprice>0){
            ...
            path[0] = WBNB;
            path[1] = AGD;
            amount2=router.getAmountsOut(1*10**18,path);
            _price=amount2[0]*10**18/amount2[1];
        }
...
function nodemarket() public view  returns(address[] memory,uint256[] memory){
        
        address[] memory viplist4= new address[](nftsl);
       uint num;
        
       (viplist4,num)=node();
       address[] memory viplist2= new address[](num);
       uint256[] memory performance2= new uint256[](num);
        for (uint n = 0; n < nftsl; n++) {
    ...
    address[] memory viplist5= new address[](10);
       uint256[] memory performance5= new uint256[](1);
    for(uint256 y=0;y<10;y++){
        if(y==0){
            performance5[y]=performance2[y];
...
function node() public view  returns(address[] memory,uint ){
   ...
   function changefundAddressA(address _AddressA) public onlyOwner {
    ...
    function setplanastart_end(uint256 logon1,uint256 pnum, uint256 bd)  public onlyOwner(){
           logon=logon1;vipcount=pnum;bdbd=bd;
    }
