...
contract FFF   {
    ...
    uint256 private constant feePercents1 = 90; 
    uint256 private constant feePercents2 = 100;
    uint256 private constant feePercents3 = 10;  
    ...
    uint256 private constant maxAddFreeze = 15 days;
    ...
    address public feeReceivers3;

    address public defaultRefer = 0x17ACbe5710CB0aA20F3fF02F65c3C55a34F03259;
   ...
   struct Profit {
    ...
    uint256 public  whereDeposit;

    constructor() public {
       
        feeReceivers1 = 0x3eD56A2E130B73e82fdE12BF6798102F096B917B; 
        feeReceivers2 = 0x365090DD7a6157AaE61f998c8040dA6241ef9F77;
        feeReceivers3 = 0x914B3973b1442E22f7E86bE17dAAbC220bd90D64;
    ...
    function deposit(uint256 _amount) external {
        require(!isReset,"System restarting");     
    ...
    function withdraw() external {
        ...
        if(isReset){
          require(userRewards.keep>0);  
          withdrawable = userRewards.keep;
        }
    ...
    function _calLevelNow(address _user) private view returns(uint256) { 
        ...
        if(total >= 100e18){
           ...
           else  if(total >= 2000e18 && user.teamNum >= 100 && maxTeam >= 50000e18 && otherTeam >= 50000e18){
                levelNow = 5;
            }else if(total >= 1500e18 &&user.teamNum >= 30 && maxTeam >= 10000e18 && otherTeam >= 10000e18){
                levelNow = 4;
            }else if(total >= 1000e18 &&user.teamNum >= 10 && maxTeam >= 5000e18 && otherTeam >= 5000e18){
                levelNow = 3;
        }else if(total >= 500e18 && user.teamNum >= 5 && maxTeam >= 2000e18 && otherTeam >= 2000e18 ){
            levelNow = 2;
    ...
    function _deposit(address _user, uint256 _amount) private { 
        ...
        require(_amount <= maxDeposit, "Greater than maximum"); 
        ...
        uint256 addFreeze = (user.orderTime.div(3)).mul(timeStep);
        ...
        uint256 profit =  _amount.mul(12).div(100);
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 fee3 = _amount.mul(feePercents3).div(baseDivider); 
        usdt.transfer(feeReceivers3, fee3);
        ...
        uint256 keep= _amount.mul(80).div(baseDivider);
    ...
    function selfRestart(address _user) private returns(bool) {
       UserInfo storage user= userInfo[msg.sender];
       if(user.orderTime>=48){ 
        user.orderTime = 0;
        _resetWhell(_user);
        return true;
       }
    ...
    function checkReset() private{
            isReset = true;
            resetTime = block.timestamp + (5*timeStep);
            luckPool = 0;
            topPool = 0;
            keepPool = 0;
            disKeepPool(); 
         ...
         uint256 differ = block.timestamp - (40 * timeStep); 
          ...
          if(holders.length > 0){
             for(uint256 i = holders.length ;i > 0 ;i--){
                uint256 userStart = userInfo[holders[i-1]].start;
                if(userStart>differ){
                    whereDeposit +=userInfo[holders[i-1]].maxDeposit;
                }else  break ;
             }
          }

          if(whereDeposit>0 && bal >0){
            for(uint256 i = holders.length - 1;i > 0 ;i--){
               UserInfo storage use =  userInfo[holders[i-1]];
                if(use.start>differ){
                   uint256 keep =  use.maxDeposit.mul(bal).div(whereDeposit);
                   use.totalRevenue += keep;
                   RewardInfo storage rewards = rewardInfo[holders[i-1]];
                   rewards.keep = keep;
                }else  break ;
             }
          }
    ...
    function systemReset()   external{
         if(isReset && block.timestamp > resetTime && holders.length>0){
          ...
          for(uint256 i=0;i<holders.length;i++){
             ...
             cleanSystemReward(holders[i]);
             removeOrders(holders[i]);
             _resetWhell(holders[i]);
          }
            isReset = false;
            resetTime = 0;
         }else return; 
    ...
    function cleanSystemUser(address _user) private{
        UserInfo storage user = userInfo[_user];
           ...
           user.totalFreezed = 0;
           user.totalFreezed  = 0;
           user.teamTotalDeposit = 0;
           
    }

    function cleanSystemReward(address _user) private{
        RewardInfo storage reward = rewardInfo[_user];
           reward.capitals = 0;
           reward.statics = 0;
           reward.directs = 0; 
    ...
    function _resetWhell(address _user) private {
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                 userWheel[upline][_user] = 0;   
                if(upline == defaultRefer) break;
                upline = userInfo[upline].referrer;
            }else{
                break;
