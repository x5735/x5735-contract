 ...
 * that, the lfMatic vault does not need any changes, not even to know the address
 * of the dripper.
...
contract Dripper is Initializable, Governable {
  ...
  ///  rate, and rebase lfMatic.
  function collectAndRebase() external {
    require(
      IVault(vault).isRebaseManager(msg.sender) || msg.sender == vault || isGovernor(),
      "Only Governor or Rebase Managers can rebase the Vault"
    );
    _collect();
    IVault(vault).rebase();
  }
