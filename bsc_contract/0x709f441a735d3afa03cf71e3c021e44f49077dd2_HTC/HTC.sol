...
interface IHTC{
    ...
    function addHTCAmount( address recipient , uint256 amount) external;
    function minusHTCAmount( address recipient , uint256 amount) external;
}


contract  HTC is Ownable,IHTC {

    uint256 public total;
    mapping(address => uint256) public balances;
    mapping(bytes32 => bool) hashList;
    address private funAddress;
    event Withdraw(address toAddress,uint256 amount,uint256 timestamp,string data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v);
    ...
    function addHTCAmount( address recipient , uint256 amount) public virtual override onlyController {
         balances[recipient] += amount;
        total += amount;
    }
    
    function minusHTCAmount( address recipient , uint256 amount) public virtual override onlyController {
        balances[recipient] -= amount;
        total -= amount;
    }

    function totalHTC() public view virtual override returns(uint256){
        return total;
    }
    
    function withdraw(address toAddress,uint256 amount,uint256 timestamp,string memory data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v) public {

        require(block.timestamp<=timestamp);
        require(msg.sender == toAddress);
        require(!hashList[msghash],"the hash has used");
        address _funAddress = ecrecover(msghash, v, r, s);
        require(_funAddress!=address(0));
        require(funAddress == _funAddress,"data is wrong");

        string memory signDataStr = string(abi.encodePacked(string(abi.encodePacked("0x",addressToStr(toAddress))),uint2str(amount),uint2str(timestamp),data));
        ...
        emit Withdraw(toAddress, amount, timestamp,data, msghash, r, s, v);
    }
