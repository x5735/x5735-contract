...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	function _get_total_user_rewards(uint256 _pool_id, address _address_user, PoolInfo memory _pool, UserInfo memory _user) internal view returns(uint256)
	{
		uint256 boosted_amount = _get_boosted_user_amount(_pool_id, _address_user, _user.staked_amount);
		uint256 pending = (boosted_amount * _pool.accu_reward_amount_per_share_e12) / 1e12;
		return pending;
	}

	//function deposit(uint256 _pool_id, uint256 _amount) public whenNotPaused nonReentrant
	function deposit(uint256 _pool_id, uint256 _amount, uint256[] memory _xnft_ids) public nonReentrant
	{
		...
		{
			uint256 pending = _get_total_user_rewards(_pool_id, address_user, pool, user) - user.reward_debt;			
		...
		user.reward_debt = _get_total_user_rewards(_pool_id, address_user, pool, user);
	...
	function harvest(uint256 _pool_id) public nonReentrant
	{
		...
		// renew reward amount
		uint256 total_user_rewards = _get_total_user_rewards(_pool_id, address_user, pool, user);
		
		uint256 pending = total_user_rewards - user.reward_debt;
		user.locked_reward_amount += pending;
		
		user.reward_debt = total_user_rewards;
	...
	function get_pending_reward_amount(uint256 _pool_id) external view returns(uint256)
	{
		...
		// getting mint reward amount without _mint_rewards
		...
		// getting pending reward amount without _get_total_user_rewards
		uint256 boosted_amount = _get_boosted_user_amount(_pool_id, address_user, user.staked_amount);
		uint256 total_user_rewards = (boosted_amount * new_pool_accu_e12) / 1e12;
		uint256 pending = total_user_rewards - user.reward_debt;
		return pending;
	}
