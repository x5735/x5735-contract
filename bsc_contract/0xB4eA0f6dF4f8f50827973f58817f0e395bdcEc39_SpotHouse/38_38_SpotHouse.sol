    ...
    SpotDex
{
    ...
    function claimForUser(address user, IMatchingEngineAMM pairManager) public onlyOwner {
        ...
        (
            ...
            uint256 baseAmount,
            uint256 basicPoint
        ) = getAmountClaimable(pairManager, user);
        if (quoteAmount > 0 || baseAmount > 0) {
            _clearLimitOrder(address(pairManager), user, basicPoint);
            ...
            _withdraw(pairManager, msg.sender, Asset.Base, baseAmount, true, pair);

            emit AssetClaimed(user, pairManager, quoteAmount, baseAmount);
        }
