...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _updateAdditionLevel(address _user) private {
        ...
        for (uint8 i = 0; i < 4; i++) {
            ...
            if (userToPurchasedAmountMapping[_user] >= 11323 * 1e18){// sb[3]) {
                _additionalLevel = 21;
            } else if (userToPurchasedAmountMapping[_user] >= 5467 * 1e18){//sb[2]) {
                _additionalLevel = 14;
            } else if (userToPurchasedAmountMapping[_user] >= 2939 * 1e18){//sb[1]) {
                _additionalLevel = 9;
            } else if (userToPurchasedAmountMapping[_user] >= 619 * 1e18){// sb[0]) {
                _additionalLevel = 5;
            }  
