...
contract KnightFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 knightAtLastUserAction; // keeps track of knight deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Knight token
    IKnightPool public immutable knightPool; //Knight pool
    ...
    //When call knightPool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositKnight(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _knightPool: knightPool contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     */
    constructor(
        IERC20 _token,
        IKnightPool _knightPool,
        address _admin,
        address _treasury
    ) {
        token = _token;
        knightPool = _knightPool;
        admin = _admin;
        treasury = _treasury;

        // Infinite approve
        IERC20(_token).safeApprove(address(_knightPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Knight Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    knightPool.withdrawByAmount(withdrawAmount);
        ...
        user.knightAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositKnight(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the KNIGHT Flexible Pool
    ...
    /**
     * @notice Withdraws from Knight Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking knight");
        staking = false;
        knightPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Knight Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in knightPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = knightPool.userInfo(address(this));
        uint256 pricePerFullShare = knightPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into knightPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            knightPool.deposit(bal, 0);
        }
    }
}