...
contract BULLSCMarket is ReentrancyGuard {
  ...
  function createVaultItem(address nftContract,uint256 tokenId,uint256 price,bool isPayable,address token) public payable nonReentrant {
    ...
    if (isPayable == true)
    {
    idToVaultItem[itemId] =  VaultItem(itemId,nftContract,tokenId,address(0),isPayable,payable(msg.sender),payable(address(0)),price,false);
    }
    else 
    {
    idToVaultItem[itemId] =  VaultItem(itemId,nftContract,tokenId,address(token),isPayable,payable(msg.sender),payable(address(0)),price,false);
  ...
  function BULLSCMarketSaleToken(
    address nftContract,uint256 itemId) public  nonReentrant {
    uint price = idToVaultItem[itemId].price;
    uint tokenId = idToVaultItem[itemId].tokenId;
    address seller = idToVaultItem[itemId].seller ;
    require(ERC20(idToVaultItem[itemId].token).balanceOf(msg.sender) >= price, "Not enough balance to complete transaction");
    ERC20(idToVaultItem[itemId].token).transferFrom(seller,msg.sender,price);
    IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
    idToVaultItem[itemId].sold = true;
    _itemsSold.increment();
  }
