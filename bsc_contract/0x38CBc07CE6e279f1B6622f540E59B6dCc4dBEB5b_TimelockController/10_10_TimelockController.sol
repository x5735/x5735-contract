...
contract TimelockController is AccessControl {
     ...
     */
    constructor(address _devWalletAddress)
        public
    {
    ...
    function predecessorCk(bytes32 predecessor) public view returns(bool, bool){
    ...
    function setWITHDRAWALFee(address _stratAddress, uint256 _WITHDRAWAL_FEE) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setWITHDRAWALFee(_WITHDRAWAL_FEE);
    }

    function setControllerFee(address _stratAddress, uint256 _controllerFee) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setControllerFee(_controllerFee);
    }

    function setbuyBackRate(address _stratAddress, uint256 _buyBackRate) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setbuyBackRate(_buyBackRate);
    }

    function setReceieveFeeAddress(address _stratAddress, address _receiveFeeAddress) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
    ...
    function setfundManager(address _stratAddress, address _fundManager) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setfundManager(_fundManager);
    }
    
    function setfundManager2(address _stratAddress, address _fundManager2) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setfundManager2(_fundManager2);
    }

    function setfundManager3(address _stratAddress, address _fundManager3) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IStrategy(_stratAddress).setfundManager3(_fundManager3);
    }

    function setAFIPerBlock(address _autofarmAddress, uint256 _inputAmt) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IAutoFarm(_autofarmAddress).setAFIPerBlock(_inputAmt);
    }

    function transferFarmOwnership(address _autofarmAddress, address _newOwner) public {
        require(hasRole(EXECUTOR_ROLE, msg.sender), "!EXECUTOR_ROLE");
        IAutoFarm(_autofarmAddress).transferFarmOwnership(_newOwner);
    }
}