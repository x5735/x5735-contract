...
contract RouteProcessor is StreamReader {
  ...
  IGniLampMinimal immutable gniLamp;
  IWETH public immutable wNATIVE;

  constructor(address _gniLamp, address _wNATIVE) {
  ...
  function processRoute(
    ...
    while (isNotEmpty(stream)) {
      ...
      if (commandCode < 20) {
        if (commandCode == 10)
          swapUniswapPool(stream); // Kami/Uniswap pool swap
        ...
        else if (commandCode == 21) swapRatio(stream);
  ...
  // Transfers input tokens from GniLamp to a pool.
  // Expected to be launched for initial liquidity distribution from user to Gni, so we know exact amounts
  ...
  // Transfers all input tokens from GniLamp to a pool
  ...
  // Withdraw Gni tokens from Gni to an address.
  ...
  // Withdraw all Gni tokens from Gni to an address.
  ...
  // Distributes input Gni tokens from msg.sender to addresses. Tokens should be approved
  // Expected to be launched for initial liquidity distribution from user to pools, so we know exact amounts
  ...
  // Distribute Gni tokens from this routeProcessor to addresses.
