...
contract PyushppWorldX2{
    ...
    event MatrixUpgrade(string  investorId,uint256 investment,address indexed investor,string matrixName);
    event ClubUpgrade(string  investorId,uint256 investment,address indexed investor,string clubNAme);
    ...
    function NewRegister(string memory referralId,string memory referral,uint256 investment) public payable
	{
    ...
    function UpgradeMatrix(string memory investorId,uint256 investment,string memory matrixName) public payable
	{
	  require(USDT.balanceOf(msg.sender)>=investment);
		require(USDT.allowance(msg.sender,address(this))>=investment,"Approve Your Token First");
		USDT.transferFrom(msg.sender ,owner,investment);
		emit MatrixUpgrade( investorId,investment,msg.sender,matrixName);
	}

    function UpgradeClub(string memory investorId,uint256 investment,string memory clubNAme) public payable
	{
	  require(USDT.balanceOf(msg.sender)>=investment);
		require(USDT.allowance(msg.sender,address(this))>=investment,"Approve Your Token First");
		USDT.transferFrom(msg.sender ,owner,investment);
		emit ClubUpgrade( investorId,investment,msg.sender,clubNAme);
	}
