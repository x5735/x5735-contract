...
contract DiceRollClassic is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 constant MAX_BET_PERCENTAG = 94;
    uint256 constant MIN_BET_PERCENTAG = 4;
    uint256 constant MOD = 100;

    event NewRandomGenerator(address indexed randomGenerator);
    event Received(address, uint);
    event BetPlace(uint256 indexed epoch,address user,uint256 requestId, uint256 rollUnder);
    ...
    function betPlace(uint256 rollUnder)
        external
        payable
        whenNotPaused
        nonReentrant
        notContract
    {
        ...
        require(rollUnder >= MIN_BET_PERCENTAG && rollUnder <= MAX_BET_PERCENTAG,"Percentag should be within range.");
        ...
        emit BetPlace(_dice.epoch,_dice.user,_dice.requestId,_dice.rollUnder);
    }
     ...
     function safeEndDice(uint256 requestId) external onlyManipulator {
        ...
        uint256 rewardAmount = _calculateReward(currentEpoch, _dice.amount, _dice.rollUnder);

        require(address(this).balance > rewardAmount);

        if (dice < _dice.rollUnder) {
    ...
    function _calculateReward(uint256 epoch,uint256 amount, uint256 rollUnder) internal returns(uint256 rewardAmount) {

        require(rolledDices[epoch].rewardAmount == 0, "Rewards calculated");

        uint256 treasuryAmt;
     
        require(0 < rollUnder && rollUnder <= MOD, "Win probability out of range.");

        treasuryAmt = (amount * treasuryFee) / 10000;

        require(treasuryAmt <= amount, "Bet doesn't even cover house treasuryAmt.");

        rewardAmount =  ((amount - treasuryAmt) * MOD) / rollUnder;
