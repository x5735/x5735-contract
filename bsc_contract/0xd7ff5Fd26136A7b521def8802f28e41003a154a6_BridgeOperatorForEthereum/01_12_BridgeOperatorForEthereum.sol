...
contract BridgeOperatorForEthereum is Ownable, Pausable {
    ...
    bytes32 public ETH_OPERATOR_ADDRESS;
    ...
    constructor(
        ...
        uint256 _pid,
        uint16 _eth_chain_id,
        bytes32 _eth_operator_address
    ) {
        cakeOft = _cakeOft;
        cake = IERC20(cakeOft.token());
        masterChefV2 = _masterChef;
        pid = _pid;
        ETH_CHAIN_ID = _eth_chain_id;
        ETH_OPERATOR_ADDRESS = _eth_operator_address;
        operator = owner();
    ...
    function setAptosOperator(bytes32 newAptosOperator) external onlyOwner {
        ETH_OPERATOR_ADDRESS = newAptosOperator;
