...
interface IPositionRouterCallbackReceiver {
    function DxpPositionCallback(bytes32 positionKey, bool isExecuted, bool isIncrease) external;
}
...
contract BasePositionManager is IBasePositionManager, ReentrancyGuard, Governable {
    ...
    // mapping (address => uint256) public override maxGlobalShortSizes;
     mapping (address => uint256) public  maxGlobalShortSizes; // edit: override specified but does not override anything.
...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function _callRequestCallback(
        ...
        bool success;
        try IPositionRouterCallbackReceiver(_callbackTarget).DxpPositionCallback{ gas: _gasLimit }(_key, _wasExecuted, _isIncrease) {
            success = true;
        } catch {}
