...
contract MetaLpStaking is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    function withdraw() public nonReentrant updateReward(msg.sender) {
        ...
        require(userBalance > 0, "You are not a LP farmer");
        ...
        if(block.timestamp >= periodFinish){
            ...
            rewardsToken.safeTransfer(msg.sender,reward.mul(remainingFeePercent).div(THOUSAND));
            totalFee += reward.mul(penaltyFee).div(THOUSAND);
        }
