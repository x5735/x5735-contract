...
contract GWC is ERC20Burnable, Brothers, LpHolder, Swap {
    ...
    function addBlacklist(address user) public onlyRole(BLACKLIST_ROLE) {
        blacklist[user] = true;
    }

    function removeBlacklist(address user) public onlyRole(BLACKLIST_ROLE) {
        blacklist[user] = false;
    }
    
    function mint(address _to, uint256 _amount) public onlyRole(MINT_ROLE) returns (bool) {
