...
import './interfaces/IBrewlabsRouter02.sol';
import './libraries/BrewlabsLibrary.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract BrewlabsRouter02 is IBrewlabsRouter02 {
    using SafeMath for uint;

    address private admin;
    address private nftDiscountManager;
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, 'BrewlabsRouter: FORBIDDEN');
        _;
    }
    ...
    function setNFTDiscountManager(address _manager) external onlyAdmin {
        nftDiscountManager = _manager;
    }
    ...
    function _addLiquidity(
        ...
        if (IBrewlabsFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IBrewlabsFactory(factory).createPair(tokenA, tokenB, feeDistribution);
        }
        (uint reserveA, uint reserveB) = BrewlabsLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = BrewlabsLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'BrewlabsRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = BrewlabsLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function _mintLPTokenTo(address tokenA, address tokenB, uint amountA, uint amountB, address to) internal returns (uint liquidity) {
    ...
    function addLiquidity(
        ...
        (amountA, amountB) = _addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, feeDistribution);
        liquidity = _mintLPTokenTo(tokenA, tokenB, amountA, amountB, to);
    }
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IBrewlabsPair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        uint discount = getNFTDiscount(msg.sender);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            uint _discount = discount;
            uint operationFee = pair.operationFee();
            (uint reserveInput, uint reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
            amountInput = IERC20(input).balanceOf(address(pair)).sub(reserveInput);
            amountOutput = BrewlabsLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, operationFee.mul(100 - _discount));
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? BrewlabsLibrary.pairFor(factory, output, path[i + 2]) : _to;
            {
            uint _discount = discount;
            pair.swap(amount0Out, amount1Out, to, new bytes(0), _discount);
            }
        }
    }
    ...
    function getNFTDiscount(address addr) internal view returns(uint256 discount) {
        if (nftDiscountManager != address(0)) {
            discount = IBrewlabsNFTDiscountManager(nftDiscountManager).getNFTDiscount(addr);
        } else {
            discount = 0;
        }
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint operationFee)
        public
        pure
        virtual
        override
    ...
    {
        return BrewlabsLibrary.getAmountOut(amountIn, reserveIn, reserveOut, operationFee);
    }

    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint operationFee)
        public
        pure
        virtual
        override
    ...
    {
        return BrewlabsLibrary.getAmountIn(amountOut, reserveIn, reserveOut, operationFee);
    }
