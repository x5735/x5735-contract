...
contract CrapyStaking {
    uint256 constant APR = 1000; // 1000% APR
    uint256 constant PENALTY = 15; // 15% penalty if unstaked before 5 days
    uint256 constant MAX_STAKE = 10000000 * 10**18; // Maximum total amount staked
    uint256 constant withdrawTime = 1680719908;
    ...
    function withdraw() external {
        ...
        if (stakeTime >= 5 days) {
            reward = (stakeAmount * APR * 5 days) / (365 days * 100);
        } else {
            reward = (stakeAmount * APR * stakeTime) / (365 days * 100);
            penalty =
                (stakeAmount * PENALTY * (5 days - stakeTime)) /
                (100 * 365 days);
        ...
        returns (uint256 reward, uint256 lockTime)
    {
        ...
        if (stakeAmount > 0) {
            reward = (stakeTime < 5 days) ?(stakeAmount * APR * stakeTime) / (365 days * 10000):(stakeAmount * APR * 5 days) / (365 days * 10000);
            lockTime = (stakeTime < 5 days) ? (5 days - stakeTime) : 0;
        } else {
            reward = 0;
            lockTime = 0;
        }
