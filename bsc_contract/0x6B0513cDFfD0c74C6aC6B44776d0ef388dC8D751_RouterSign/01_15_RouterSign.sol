...
interface IVaultPriceFeedV3 {
    function updateWithSig(uint256[] memory _priceBits, uint256 _priceTimestamp,  address _updater, bytes memory _updaterSignedMsg) external;
}
...
contract RouterSign is Ownable, ReentrancyGuard {
    ...
    function increasePosition(address[] memory _path, address _indexToken, uint256 _amountIn, uint256 _sizeDelta, bool _isLong, uint256 _price,
            uint256[] memory _priceBits, uint256 _priceTimestamp,  address _updater, bytes memory _updaterSignedMsg) external payable nonReentrant{
        IVaultPriceFeedV3(priceFeed).updateWithSig(_priceBits, _priceTimestamp,  _updater, _updaterSignedMsg);
        if (_amountIn > 0) {
