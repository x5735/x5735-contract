    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    function availableRewards(uint256 id) public view returns (uint256) {
        ...
        if (totalClaimed + rewards > maxPayout) {
            rewards = maxPayout - totalClaimed < 0
                ? 0
                : maxPayout - totalClaimed;
        }
        if (totalClaimed + rewards > (realValue * maxROI) / 100) {
            rewards = (realValue * maxROI) / 100 - totalClaimed < 0
                ? 0
                : (realValue * maxROI) / 100 - totalClaimed;
        }
        return rewards;
    }
