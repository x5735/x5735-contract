...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	function deposit(uint256 _pool_id, uint256 _amount) public nonReentrant
	{
		...
		{
			uint256 pending = (_get_boosted_user_amount(_pool_id, address_user, user.staked_amount) * pool.accu_reward_amount_per_share_e12 / 1e12) - user.reward_debt;
			if(pending > 0)
	...
	function deposit_nfts(uint256 _pool_id, uint256[] memory _xnft_ids) public nonReentrant
	{
		...
		if(user.staked_amount > 0)
		{
			uint256 pending = (user.staked_amount * pool.accu_reward_amount_per_share_e12 / 1e12) - user.reward_debt;
			if(pending > 0)
				user.locked_reward_amount += pending;
		}
		...
		{
			// User -> Bullish
			amount_wei = _deposit_nfts(_pool_id, _xnft_ids) * 1e18;
	...
	function withdraw_nfts(uint256 _pool_id, uint256[] memory _xnft_ids) public nonReentrant
	{
		...
		{
			// Bullish -> User
			amount_wei = _withdraw_nfts(_pool_id, _xnft_ids) * 1e18;
