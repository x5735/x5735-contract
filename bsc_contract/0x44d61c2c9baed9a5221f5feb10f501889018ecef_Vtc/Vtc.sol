...
contract Vtc is ERC20, Ownable {
    ...
    VtcDividendTracker public dividendTracker;
    ...
    constructor() ERC20("VAULTCHAIN", "VTC") {
        marketingWallet = payable(0x34C54DD0270ED0398497c03AE23fa70514dBc5CE);
        devWallet = payable(0x9387F50B8eE5f3Eb7C98A76e0F53A79E01707Fa9);
        ...
        dividendTracker = new VtcDividendTracker(
            payable(this),
            router,
...
contract VtcDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "Vtc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Vtc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(VtcContract);

            uint256 prevBalance = VtcContract.balanceOf(address(this));
            ...
            {
                uint256 received = VtcContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    VtcContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
