...
contract LuckyKeys is Auth {
    ...
    string constant public name = "Mini Luckys Keys Game";
    string constant public symbol = "Mini LKY";
    uint256 constant private rndInit_ = 4 hours;                
    uint256 constant private rndInc_ = 30 seconds;              
    uint256 constant private rndMax_ = 4 hours;    
    uint256 constant private rndEnd_ = 7 days;            
    
    bool internal locked;

    uint256 public rID_;    

    uint256 public potPerc = 45;
    uint256 public keyHolderPerc = 20; 
    uint256 public devPerc = 3; 
    uint256 public markPerc = 3; 

    uint256 public keyBuytokenHoldersPerc = 15; 
    uint256 public keyBuyLuckeyBuybackPerc = 7;
    uint256 public keyBuytokenBuybackPerc = 7;
    
    uint256 public roundEndtokenHoldersPerc = 5; 
    uint256 public roundEndLuckeyBuybackPerc = 15;
    uint256 public roundEndtokenBuybackPerc = 4;
    uint256 public roundEndRolloverPerc = 5;
    

    uint256 public endGameKeyReq = 15000000000000000000000;
    uint256 public minTokenBalance = 300000000000000000000;
    address public devFeeReceiver; 
    address public marketingFeeReceiver;
    address public tokenBuybackFeeReciever;
    address public mainKeysBuybackFeeReciever;
    ...
    event roundEndInternalEvent(address indexed from, uint256 roundID,address winnerAddy,uint256 potAmount,uint256 winnerAmount,
                        uint256 keyHolderAmount,uint256 nextPotAmount);
    event roundEndExternalEvent(address indexed from, uint256 roundID, uint256 tokenHolderAmount, uint256 mainKeysBuybackAmount, 
                        uint256 tokenBuyBackAmount, uint256 marketingAmount, uint256 devAmount);
    ...
    constructor(address _luckyToken,address _diviesAddy)Auth(msg.sender) {
        ...
        mainKeysBuybackFeeReciever = msg.sender;
        ...
        returns(uint256 ,uint256)
    {
        ...
        if (block.timestamp > round_[_rID].end && round_[_rID].ended == false && round_[_rID].plyr != address(0) || block.timestamp > round_[_rID].forceEnd && round_[_rID].ended == false && round_[_rID].plyr != address(0))
        {
            if (round_[_rID].plyr == playerAddy)
            {
                return
                (
                    (plyr_[playerAddy].win).add( ((round_[_rID].pot).mul(potPerc)) / 100 ),
    ...
    function getPlayerVaultsHelper(address playerAddy, uint256 _rID)
        ...
        returns(uint256)
    {
        return(  ((((round_[_rID].mask).add(((((round_[_rID].pot).mul(keyHolderPerc)) / 100).mul(1000000000000000000)) / (round_[_rID].keys))).mul(plyrRnds_[playerAddy][_rID].keys)) / 1000000000000000000)  );
    }
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _devFeeReceiver, address _tokenBuybackFeeReciever, address _mainKeysBuybackFeeReciever) external authorized {
        marketingFeeReceiver = _marketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        tokenBuybackFeeReciever = _tokenBuybackFeeReciever;
        mainKeysBuybackFeeReciever = _mainKeysBuybackFeeReciever;
    }

    

    function buyCore(address playerAddy,bool deposit)
        private
    {
        uint256 _rID = rID_;
        ...
        if (_now > round_[_rID].strt && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == address(0))) && (_now <= round_[_rID].forceEnd || (_now > round_[_rID].forceEnd && round_[_rID].plyr == address(0)))) 
        {
            if(canIBuyKeys(playerAddy) || isDeposit ){
                core(_rID, playerAddy, msg.value, isDeposit);
            } else{
                refund = true;
            }
    ...
    function core(uint256 _rID, address playerAddy, uint256 _eth,bool _deposit)
        private
    {
    ...
    function distributePotExternal(uint256 _rID, uint256 _pot) private {
        uint256 _mark = (_pot.mul(markPerc)) / 100;
        uint256 _bb = (_pot.mul(roundEndtokenBuybackPerc)) / 100;
        uint256 _mainKeyBB = (_pot.mul(roundEndLuckeyBuybackPerc)) / 100;
        uint256 _tok = (_pot.mul(roundEndtokenHoldersPerc)) / 100;
        ...
        require(bbsuccess, "Transfer failed.");
         //pay mainKeyBB
        (bool keysucess, ) = mainKeysBuybackFeeReciever.call{value:_mainKeyBB}("");
        require(keysucess, "Transfer failed.");

        if (_tok > 0)
            Divies.deposit{ value: _tok }();

        emit roundEndExternalEvent (address(0),_rID,_tok,_mainKeyBB,_bb,_mark,_mark);
    }
        ...
        private
    {
        ...
        uint256 _win = (_pot.mul(potPerc)) / 100;
        uint256 _gen = (_pot.mul(keyHolderPerc)) / 100;
        uint256 _res = (_pot.mul(roundEndRolloverPerc)) / 100;
        ...
        distributePotExternal(_rID,_pot);
         
        emit roundEndInternalEvent(address(0),_rID,winnerAddy,_pot,_win,_gen,_res);
    ...
    function distributeExternal(uint256 _eth)
        private
    {
        uint256 _dev = _eth.mul(devPerc).div(100); 
        uint256 _mark = _eth.mul(markPerc).div(100); 
        uint256 _tokenBB = _eth.mul(keyBuytokenBuybackPerc).div(100); 
        uint256 _mainKeyBB = _eth.mul(keyBuyLuckeyBuybackPerc).div(100);
        uint256 _tok = _eth.mul(keyBuytokenHoldersPerc).div(100);
       
        payable(marketingFeeReceiver).transfer(_mark);
        payable(devFeeReceiver).transfer(_dev);
        payable(tokenBuybackFeeReciever).transfer(_tokenBB);
        payable(mainKeysBuybackFeeReciever).transfer(_mainKeyBB);
        ...
        private
    {
        ...
        uint256 _gen = (_eth.mul(keyHolderPerc)) / 100;
        ...
        returns(uint256)
    {
        ...
        uint256 _earnings = (plyr_[playerAddy].win).add(plyr_[playerAddy].gen);
        if (_earnings > 0)
        {
    ...
    function endKeys(uint256 _eth) 
        ...
        returns(uint256)
    {
        return ((((((_eth).mul(1000000000000000000)).mul(3125000000000000000000000000000)).add(5624988281256103515625000000000000000000000000000000000000000000)).sqrt()).sub(74999921875000000000000000000000)) / (1562500000000);
    ...
    function endEth(uint256 _keys) 
        ...
        returns(uint256)  
    {
        return ((781250000000).mul(_keys.sq()).add(((149999843750000).mul(_keys.mul(1000000000000000000))) / (2))) / ((1000000000000000000).sq());
...
library datasets {
    ...
    struct Round {
        ...
        uint256 forceEnd; // round will end at this date (game WILL NOT go on forever)
    }
}
