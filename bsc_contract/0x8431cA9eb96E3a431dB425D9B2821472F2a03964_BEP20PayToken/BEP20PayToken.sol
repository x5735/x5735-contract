...
contract BEP20PayToken is Ownable{
    ...
    event PayToken(address indexed token, address indexed sender, uint indexed amount, uint types);
    ...
    function payToken(address token, uint amount, uint types) external returns(bool){
        ...
        emit PayToken(token, sender, amount, types);
