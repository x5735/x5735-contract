...
contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xA07Bec378605b26D56D8756da37F9e6896783a2D;
...
contract BabyChicken is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "BABY-KFC"; //    
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 feeAmount = amount * (_marketingFee + _liquidityFee) / 100;
    ...
    function _transfer(
        ...
        if(contractTokenBalance >= _maxTxAmount)
        {
         ...
         if(from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxTxAmount / txSelling, "Transfer amount exceeds the maxTxAmount.");
