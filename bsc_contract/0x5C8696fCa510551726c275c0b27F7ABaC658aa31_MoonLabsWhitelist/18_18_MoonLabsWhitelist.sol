...
interface IMoonLabsWhitelist {
  function getIsWhitelisted(address _address) external view returns (bool, bytes memory);
}
...
contract MoonLabsWhitelist is Initializable, IMoonLabsWhitelist, OwnableUpgradeable {
   ...
   */
  function getIsWhitelisted(address _address) public view override returns (bool, bytes memory) {
    bool valid;
    bytes memory error;

    if (tokenToWhitelist[_address]) return (true, error);

    /// Check for v2 pairs
    (valid, error) = _checkV2Pair(_address);
    if (valid) return (true, error);

    /// Check for v3 pools
    (valid, error) = _checkV3Pool(_address);
    if (valid) return (true, error);
    return (false, error);
  }
   ...
   */
  function _checkV2Pair(address _address) private view returns (bool, bytes memory) {
    bytes memory error;
    ...
    try pairContract.token0() returns (address _token0) {
      if (_checkIfValidPair(_token0)) return (true, error);
    } catch (bytes memory _error) {
      error = _error;
      return (false, error);
    }
    try pairContract.token1() returns (address _token1) {
      if (_checkIfValidPair(_token1)) return (true, error);
    } catch (bytes memory _error) {
      error = _error;
      return (false, error);
    }
   ...
   * @param _address address to check
   */
  function _checkV3Pool(address _address) private view returns (bool, bytes memory) {
    bytes memory error;
    /// Check if address is contract
    if (_address.code.length == 0) {
      return (false, error);
