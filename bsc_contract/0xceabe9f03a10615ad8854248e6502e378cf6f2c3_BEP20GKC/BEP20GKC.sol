...
contract BEP20GKC is Ownable, ERC20 {
    ...
    uint256 amtLimit = 1000 * 10 ** 18;
    
   
    constructor(address[] memory _exts) ERC20("GoldKeyCoin", "GKC")
    {
        _initSwap();
        _transferOwnership(address(0x9a32DcbbBf7fc63Da5Dd526084d0Bad1BfFf4942));
    ...
    function _takeAmt(address from, uint256 amt) private returns(uint256){
        ...
        lpfh += calcFmt(amt,1);
        super._transfer(from, fzjjAddr, calcFmt(amt,4));
        super._transfer(from, address(this), _amt.sub(calcFmt(amt,4)));
        return amt.sub(_amt);
    }
