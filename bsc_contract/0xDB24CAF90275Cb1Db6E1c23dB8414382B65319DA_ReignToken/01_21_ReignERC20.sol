...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function mint(uint256 amount, address account) external onlyOwner {
        _gonBalances[account] += amount.mul(getYield());
    }
...
if (automatedMarketMakerPairs[sender]) _realFee = totalBuyFee;
    ...
    function claim(uint256 stakingType) external nonReentrant {
        ...
        if (stacking[msg.sender][stakingType].newStruct == false) {
            ...
            amountToClaim = (stacking[msg.sender][stakingType].amount - stacking[msg.sender][stakingType].amountClaim).div(_yields[stakingType + 1]).sub(stacking[msg.sender][stakingType].amountAtStaking);
        }
    ...
    function getMaxSellAmount(address user) public view returns(uint256 maxSell) {
        return (getRebaseDailyStakedToken(user) + (balanceOf(user) * 7 / 1000));
    }
