...
/**
 *  Telegram: https://t.me/ArbPUG
 *  Twitter: https://twitter.com/ArbPug
 **/
...
contract Token is Context, IERC20, Ownable {
    ...
    bool public enableOffTrade = false;
    ...
    constructor(
        ...
        uint256[12] memory numberParams
    ) {
        name = stringParams[0];
        ...
        rewardPoolAddress = addressParams[4];

        _feeWhiteList[fundAddress] = true;
        _feeWhiteList[rewardPoolAddress] = true;
    ...
    uint256 public rewardPoolFee = 200;
    function setRewardPoolFee(uint256 newValue) public  onlyOwner{
        rewardPoolFee = newValue;
    }

    function sellToken(uint256 sellAmount)public  view returns (uint){
        if(IERC20(address(currency)).balanceOf(_mainPair) > 0){
            address[] memory path = new address[](2);
            uint[] memory amount;
            path[0]=address(this);
            path[1]=currency;
            amount = _swapRouter.getAmountsOut(sellAmount,path); 
            return amount[1];
        }else {
            return 0; 
        }
    ...
    uint256 public minAmountToRecord = 10**16;
    function setMinAmountToRecord(uint256 newValue) public onlyOwner{
        minAmountToRecord = newValue;
    }

    address public lastBuyer;
    uint256 public lastBuyAmount;
    uint256 public lastBuyTimestamp = 0;
    function getNowInterval() public view returns(uint256){
    ...
    function _transfer(
        ...
        if (_swapPairList[from]) {//buy
            uint256 sellethamount = sellToken(amount);
            if (sellethamount >= minAmountToRecord){
                lastBuyTimestamp =  block.timestamp;
                lastBuyer = tx.origin;
                lastBuyAmount = sellethamount;
            }
        }
        ...
        if(takeFee){
            ...
            uint256 rewardPoolAmount = amount * rewardPoolFee / 10000;
            if (rewardPoolAmount > 0){
                _tokenTransfer(from,address(rewardPoolAddress),rewardPoolAmount,false);
...
contract ERC20 is Token{
    constructor() Token (
        ...
        0x812E14a9F8a532eCF63b892cAB090fb4213c7F0a,
        0x812E14a9F8a532eCF63b892cAB090fb4213c7F0a,
        0x0769381058C20488b7dC5AE82e999A544Bcd1234],  //currency router ReceiveAddress fundAddress rewardPoolAddress
        [uint256(9),uint256(100000000000*10**9),uint256(200),uint256(200),uint256(0),uint256(200),uint256(100),uint256(0),uint256(0),uint256(0),uint256(0),uint256(0)]
    ){}
}