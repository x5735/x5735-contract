...
contract ClaimReward is Pausable, Ownable {
    ...
    mapping(address => LastClaim) public lastClaimedTime;
    mapping(address => ClaimInfo[]) public ClaimHistory;

    event ClaimedReward(address recipient, address tokenAddress, uint256 amount, uint256 blocktime);
    ...
    function claim(
        ...
        if (lastClaimedTime[msg.sender].isVal) {
            require(lastClaimtime == lastClaimedTime[msg.sender].blocktime, "This claim is not match with last cliamed time.");
        }
        ...
        lastClaimedTime[msg.sender] = LastClaim(blocktime, true);
        // save claim history
        ClaimHistory[msg.sender].push(ClaimInfo(msg.sender, tokenAddress, amount, request.nonce, blocktime));

        emit ClaimedReward(msg.sender, tokenAddress, amount, blocktime);
    }
