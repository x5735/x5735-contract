...
contract ProtonStaking is Ownable, ReentrancyGuard{
    ...
    uint256 public devFee = 300; // 3% dev
    uint256 public ref = 500; // 5% refer
    uint256 public period = 1 days;
    uint256 public capitalPeriod = 60 * 60 * 24 * 20; // lock per 20 days
    uint256 public apr = 400; // 100 =  1% per day
    uint256 public percentRate = 10000;

    address private devWallet;
    address public ProtonContract;
    ...
    constructor(address _feesWallet, address _tokenContract) {
        ...
        require(_tokenContract!=address(0),"Please provide a valid Proton contract address");
       
        devWallet = _feesWallet;
        ProtonContract = _tokenContract;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        require(_amount > 0, "you can deposit more than 10 Proton");

        if(_referrer == msg.sender){
            _referrer = address(0);
        }
        IERC20(ProtonContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(ProtonContract).safeTransfer(devWallet,depositFee);
        // transfer 5% fee to referrer
        ...
        if(investors[msg.sender].referrer != address(0)){
            ...
            IERC20(ProtonContract).transfer(investors[msg.sender].referrer, referrerAmount);
        }
    ...
    function claimBonus() public {
        ...
        IERC20(ProtonContract).safeTransfer(msg.sender, _bonus[msg.sender]);
        _bonus[msg.sender] = 0;
	
    }

    function setBonus(address _addr, uint256 amount) public onlyOwner{
        _bonus[_addr] += amount;
    }

    function revokeBonus(address _addr, uint256 amount) public onlyOwner{
        _bonus[_addr] -= amount;
    }

    function changeApr(uint256 _newapr) public onlyOwner{
        apr = _newapr;
    }
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(ProtonContract).safeTransfer(msg.sender,amountToSend);
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(ProtonContract).balanceOf(address(this));
    }
