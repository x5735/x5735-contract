...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        address RouterAddress, address bnbAddress,
        ...
        IERC20(bnbAddress).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), bnbAddress);
        _mainPair = swapPair;
        ...
        holderRewardCondition = 30 * 10 ** IERC20(bnbAddress).decimals();

        _tokenDistributor = new TokenDistributor(bnbAddress);
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        IERC20 bnb = IERC20(_bnb);
        uint256 bnbBalance = bnb.balanceOf(address(_tokenDistributor));
        uint256 fundAmount = bnbBalance * (_buyFundFee + _sellFundFee) * 2 / swapFee;
        bnb.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        bnb.transferFrom(address(_tokenDistributor), address(this), bnbBalance - fundAmount);

        if (lpAmount > 0) {
            uint256 lpFist = bnbBalance * lpFee / swapFee;
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _bnb, lpAmount, lpFist, 0, 0, address(0xB012A06Ef195B2692884A3738A0e9B94d65495A2), block.timestamp
                );
            }
        }
    }
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = bnb.balanceOf(address(this));
...
contract Yorkie is AbsToken {
    constructor() AbsToken(
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E),//RouterAddress
        address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c),//bnbAddress
        "Yorkie",//Name
        "Yorkie",//Symbol
        9,//Decimals
        1000000000000000,//Supply
        address(0xB012A06Ef195B2692884A3738A0e9B94d65495A2),//FundAddress
        address(0x4eb3fe22bC0CCA911099DaEA2308165e1261281C)//ReceiveAddress
    ){
    }
}