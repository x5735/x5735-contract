...
//Deployed with help of CoinFundBSC
//CF Telegram: https://t.me/coinfundbsc/
...
contract THECEO is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _MrktFee,uint256 _DevFee, uint256 _CFFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _MrktFee,uint256 _DevFee, uint256 _CFFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _MrktReceiver,address _DevFeeReceiver, address _CFFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "Chief Executive Officer";
    string constant private _symbol = "THECEO";
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(MrktFee).add(DevFee).add(CFFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMrkt).add(sellFeeDev).add(sellFeeCF).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address public autoLiquidityReceiver = 0xb02bD8350A31B45Df3c43DF55D245BAa16a8241A;
    address public MrktFeeReceiver = 0x75553fEDD8cEe9E12cdb4C217d4587ee6Efaa261;
    address public DevFeeReceiver = 0x75553fEDD8cEe9E12cdb4C217d4587ee6Efaa261;
    address public CFFeeReceiver = 0xb02bD8350A31B45Df3c43DF55D245BAa16a8241A;
    ...
    function clearStuckBalance() external {
        payable(CFFeeReceiver).transfer(address(this).balance);
    }

    function rescueERC20(address tokenAddress, uint256 amount) external returns (bool) {
        return ERC20(tokenAddress).transfer(CFFeeReceiver, amount);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHCF = amountETH.mul(CFFee.add(sellFeeCF)).div(totalETHFee);

        (bool tmpSuccess,) = payable(MrktFeeReceiver).call{value: amountETHMrkt}("");
        (tmpSuccess,) = payable(DevFeeReceiver).call{value: amountETHDev}("");
        (tmpSuccess,) = payable(CFFeeReceiver).call{value: amountETHCF}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _MrktFee, uint256 _DevFee, uint256 _CFFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        MrktFee = _MrktFee;
        DevFee = _DevFee;
        CFFee = _CFFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_MrktFee).add(_DevFee).add(_CFFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _MrktFee,_DevFee, _CFFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _MrktFee,uint256 _DevFee, uint256 _CFFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMrkt = _MrktFee;
        sellFeeDev = _DevFee;
        sellFeeCF = _CFFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_MrktFee).add(_DevFee).add(_CFFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _MrktFee,_DevFee, _CFFee, _BurnFee);
    }


    function setFeeReceivers(address _autoLiquidityReceiver, address _MrktFeeReceiver,address _DevFeeReceiver, address _CFFeeReceiver, address _BurnFeeReceiver) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _MrktFeeReceiver != address(0) && _DevFeeReceiver != address(0) && _CFFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        MrktFeeReceiver = _MrktFeeReceiver;
        DevFeeReceiver = _DevFeeReceiver;
        CFFeeReceiver = _CFFeeReceiver;
        BurnFeeReceiver = _BurnFeeReceiver; 

        emit SetFeeReceivers(_autoLiquidityReceiver, _MrktFeeReceiver, _DevFeeReceiver, _CFFeeReceiver, _BurnFeeReceiver);
    }
