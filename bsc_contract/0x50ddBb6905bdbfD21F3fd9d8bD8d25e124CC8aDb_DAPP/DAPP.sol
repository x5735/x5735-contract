...
contract DAPP is Ownable {
    ...
    function info(address token) public view returns(string memory symbol, uint8 decimals, uint256 allowance, uint256 balance_user, uint256 balance_contract, uint256 _principal, uint256 _dynamic, uint256 _static, uint256 _all){
        symbol = IERC20(token).symbol();
        decimals = IERC20(token).decimals();
        allowance = IERC20(token).allowance(address(this), tx.origin);
        balance_user = IERC20(token).balanceOf(tx.origin);
        balance_contract = IERC20(token).balanceOf(address(this));
        _principal = p_principal[tx.origin][token];
        _dynamic = p_dynamic[tx.origin][token];
        _static = p_static[tx.origin][token];
        _all = p_all[token];
    }
