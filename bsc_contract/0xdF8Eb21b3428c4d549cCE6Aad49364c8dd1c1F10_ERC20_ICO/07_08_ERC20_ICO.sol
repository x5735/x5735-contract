...
contract ERC20_ICO is Ownable {
    ...
    function buy(uint256 tokenAmount, address _paymentCoin) public returns (bool) {
        ...
        require(usdAmount >= 1000000000000000000 * 100, "Too small purchase!");
        require(isPaymentToken[_paymentCoin], "Token is not acceptable!");

        IERC20 stableCoin = IERC20(_paymentCoin);

        require(stableCoin.balanceOf(msg.sender) >= tokenAmount, "Not enough Token in Wallet!");
        ...
        require(baseToken.balanceOf(address(this)) >= _token, "Not enough SBK!");
    ...
    function sell(address addr, uint256 amount) external onlyOwner {
        require(baseToken.balanceOf(address(this)) >= amount, "Not enough SBK!");
     ...
     */
    function withdrawSBK(uint256 amount) external onlyOwner {
        uint256 am = amount;
        if(amount == 0 ) {
            am = baseToken.balanceOf(address(this));
        }
        baseToken.safeTransfer(wallet, am);
    }
    ...
    function withdrawUSDT(uint256 amount) external onlyOwner {
        uint256 am = amount;
        if(amount == 0 ) {
            am = IERC20(USDT_ADDRESS).balanceOf(address(this));
        }
        IERC20(USDT_ADDRESS).safeTransfer(wallet, am);
    }
    ...
    function withdrawTokens(address _token, uint256 amount) external onlyOwner {
        ...
        if(_token == address(0)) {
            ...
            uint256 am = amount;
            if(amount == 0 ) {
                am = IERC20(_token).balanceOf(address(this));
            }
            IERC20(_token).safeTransfer(wallet, am);
        }
    }
}