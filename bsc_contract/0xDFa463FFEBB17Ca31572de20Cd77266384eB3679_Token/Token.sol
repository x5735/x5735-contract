...
contract Token is IERC20, Ownable {
    string private _name = "DDCM";
    string private _symbol = "DDCM";
    ...
    uint256 public _buyHolderDividendFee = 150;
    uint256 public _sellFundFee = 100;
    uint256 public _sellHolderDividendFee = 150;

    address public _mainPair;
    address private receiveAddress;

    uint256 public _tradeTag;
    uint256 public fireT;
    mapping(address => bool) public _fires;
    ...
    constructor(
        ...
        uint256 total = 620000000 * tokenUnit;
        _tTotal = total;
        ...
        holderCondition = 220000 * tokenUnit;
        holderRewardCondition = 20 * 10 ** IERC20(usdt).decimals();
        holderRewardTotal = total;
    }
    ...
    function setBuyFee(
        uint256 buyFundFee,
        uint256 buyHolderDividendFee
    ) external onlyOwner {
        _buyFundFee = buyFundFee;
        _buyHolderDividendFee = buyHolderDividendFee;
    }

    function setSellFee(
        uint256 sellFundFee,
        uint256 sellHolderDividendFee
    ) external onlyOwner {
        _sellFundFee = sellFundFee;
        _sellHolderDividendFee = sellHolderDividendFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            require(_tradeTag != 2, "disableTransfer");
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (takeFee) {
                require(_tradeTag == 1, "disableTrade");
            }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {
                if (fireT > 0 && block.number <= fireT) {
                    _fires[recipient] = true;
                }
                // Buy
                ...
                } else if (_swapPairList[recipient] || _tradeTag == 1) {
                    // Sell Or Transfer
                    uint256 fundAmount = (tAmount *
    ...
    function swapTokenForFundAndHolder(
        uint256 tokenAmount
    ) private lockTheSwap {
        if (0 == tokenAmount) {
            return;
    ...
    function setTradeTag(uint256 tag, uint256 num) public onlyOwner {
        _tradeTag = tag;
        if (tag == 1) {
            fireT = block.number + num;
        }
    }
    ...
    function setHolder(address adr, bool flag) private {
        ...
        if (flag == true) {
            ...
            if (holders.length > 0 && holders[0] == adr) return;
        } else {
            if (holderIndex[adr] == 0) return;
        }
        uint256 size;
    ...
    uint256 public holderRewardTotal;
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance >= holdCondition && !excludeHolder[shareHolder]) {
                amount = (balance * tokenBalance) / holderRewardTotal;
    ...
    function setHolderRewardTotal(uint256 total) external onlyOwner {
        holderRewardTotal = total;
    }
