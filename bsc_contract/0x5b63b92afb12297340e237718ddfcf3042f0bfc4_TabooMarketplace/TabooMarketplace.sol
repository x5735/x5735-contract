...
contract TabooMarketplace is Ownable, ReentrancyGuard {
    ...
    function setRoyaltyContract(IRoyaltyStorage _royaltyStorage) public onlyOwner {
        royaltyStorage = _royaltyStorage;
    }
    ...
    /*
     * NFTs in a batch must contain between 2 and 1000 NFTs
     */
        ...
        auctionOngoing(_nftContractAddress, _tokenId)
    {
        ...
        _makeBid(_nftContractAddress, _tokenId, amount);
    }
    ...
    function settleAuction(
        address _nftContractAddress,
        uint256 _tokenId
    ) external nonReentrant isAuctionOver(_nftContractAddress, _tokenId) {
    ...
    function _setupSale(
        ...
        require(_erc20Token != address(0), "Only ERC20 token sale is allowed");
        nftContractSale[_nftContractAddress][_tokenId].ERC20Token = _erc20Token;
    ...
    function buyNFT(
        ...
        require(
            ...
            "Balance Of Token Must be greater than NFT cost"
        );
    ...
    function setTabooPercentageCut(
        uint256 _tabooPercentageCut
    ) external onlyOwner {
        tabooPercentageCut = _tabooPercentageCut;
    }
    ...
    function withdrawAuction(
        address _nftContractAddress,
        uint256 _tokenId
    ) external nonReentrant onlyNftSeller(_nftContractAddress, _tokenId) {
        address _nftRecipient = nftContractAuctions[_nftContractAddress][
            _tokenId
        ].nftSeller;
        address auctionERC20Token = nftContractAuctions[_nftContractAddress][
    ...
    function withdrawSale(
        ...
        address nftSeller = nftContractSale[_nftContractAddress][_tokenId]
            .nftSeller;
        require(
            nftSeller == msg.sender || msg.sender == owner(),
