...
contract AmpleNeuroNet is Ownable, ReentrancyGuard{
    ...
    uint private developerFee = 500; 
    uint private referrerReward1lvl = 500; 
    uint private referrerReward2lvl = 300; 
    uint private rewardPeriod = 1 days;
    uint public APR =  521; 
    uint private percentRate = 10000;
    ...
    constructor() {
        devWallet = 0x05ef395384b59Ab4B7B5c2ff182Fcb4b732b19D3;
    }
    ...
    function invest(uint _amount, address _referrer) external payable {
    ...
    function claim() public nonReentrant {
        require(ownedDeposits[msg.sender].length > 0, "error");
    ...
    function getAllClaimableReward(address _investor) internal view returns (uint) {
    ...
    function currentBalance() internal view returns(uint) {
        return IERC20(BUSD).balanceOf(address(this));
    }

    function getTotalRewards() internal view returns (uint) {
        return totalReward;
    }

    function getTotalInvests() internal view returns (uint) {
    ...
    function withdraw(uint _amount) external onlyOwner {
        ...
        IERC20(BUSD).safeTransfer(msg.sender, _amount);
    }
