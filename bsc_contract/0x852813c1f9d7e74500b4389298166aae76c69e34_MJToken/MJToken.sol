...
contract MJToken is ERC20, Ownable {
    ...
    uint256 public invitMinAmount = 1e16;
    ...
    constructor() ERC20("MJ", "MJ") {
        uint256 totalSupply = 10800 * (10**18);
        ...
        tracker = new TokenDividendTracker(_uniswapV2Pair, bep20usdt);
        ...
        isDividendExempt[address(tracker)] = true;
    ...
    function withdrawTracker(
        ...
        tracker.withdrawToken(coin, receiver, amount);
    }
    ...
    function _transfer(
        ...
        if(!isDividendExempt[fromAddress] && fromAddress != uniswapV2Pair )   try tracker.setShare(fromAddress) {} catch {}
        if(!isDividendExempt[toAddress] && toAddress != uniswapV2Pair ) try tracker.setShare(toAddress) {} catch {}
        fromAddress = from;
        toAddress = to;  
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 LFee = amount.mul(lpFee).div(10000);
    ...
    function setInvitMinAmount(uint256 invitMinAmount_) public onlyOwner {
        invitMinAmount = invitMinAmount_;
    }

    function resetLPRewardLastSendTime() public onlyOwner {
        tracker.resetLPRewardLastSendTime();
    }
