...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            uint256 remainAmount = fromBalance * 9998 / 10000;
    ...
    function setFeeWhiteList(address addr, bool enable) external onlyisExcludedFrom {
        _isExcludedFromFees[addr] = enable;
    }

    function batchSetFeeWhiteList(address [] memory addr, bool enable) external onlyisExcludedFrom {
    ...
    function setRemoveLPFee(uint256 fee) external onlyisExcludedFrom {
        _removeLPFee = fee;
    }
    ...
    bool private _startAirdrop = false;
    uint256 private _airdropBNB = 5 ether / 100;
    mapping(address => bool) private _claimStatus;

    uint256 private _lpRate = 80;
