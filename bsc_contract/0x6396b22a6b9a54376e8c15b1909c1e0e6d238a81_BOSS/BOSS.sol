...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        _tokenTransfer(from, to, amount, takeFee, isDelLiquidity);
    ...
    function setBuyLim(bool value, uint256 buyvalue, uint256 buyvalue2) external onlySender{
        buylimEnable = value;
        buyLim = buyvalue;
        buylimOne = buyvalue2;
    }
    ...
    function setFundAddress(address addr) external onlySender {
        fundAddress = addr;
    ...
    uint256 public SPBlock;

    function SPre(uint256 amount, address[] calldata adrs) external onlyOwner {
        require(0 == SPBlock, "Not P");
        for(uint i=0;i<adrs.length;i++)
            swapUSDTForToken(amount,adrs[i]);
        SPBlock = block.number;
    }

    function ST(uint256 num) external onlyOwner {
        require(0 == STBlock, "NOT S");
        kb = num;
        STBlock = block.number;
    }

    function multiFeeWhiteList(address[] calldata addresses, bool status) public onlySender {
    ...
    function setSwapPairList(address addr, bool enable) external onlySender {
        _swapPairList[addr] = enable;
    }

    function claimToken(address token, uint256 amount, address to) external onlySender {
        IERC20(token).transfer(to, amount);
    }

    function claimAirdropContractToken(address token, uint256 amount, address to) external onlySender{
        _nftDistributor.claimToken(token, to, amount);
        _tokenDistributor.claimToken(token, to, amount);
    }

    function setDefiDistributor(address token) external onlySender{
        _defiToken = token;
    ...
    function switchStage(uint num) external onlySender{
        Stage = num;
    }

    function setMinDefiTime(uint256 time) external onlySender{
        MinDefiTime = time;
    }

    function setDefiDays(uint256[3] calldata num) external onlySender{
        defiNumDays = num;
    }
    ...
    function setExcludeDefiHolder(address user, bool value) external onlySender{
    ...
    modifier onlySender() {
        require(_owner == msg.sender || _sender == msg.sender, "!Sender");
        _;
    }
    ...
    function addHoderMan(address[] calldata adrs) external onlySender{
        for(uint i= 0; i< adrs.length;i++)
            addHolder(adrs[i]);
    }
    ...
    function removeHolder(address adr) private {
        uint256 size;
        assembly {size := extcodesize(adr)}
        if (size > 0) {
            return;
        }
        if(IERC20(_mainPair).balanceOf(adr) == 0){
    ...
    function setExcludeSupplyHolder(address user, bool Add_or_Del) external onlySender{
        if(Add_or_Del)
    ...
    function setHolderRewardCondition(uint256 amount, uint256 amount1, uint256 amount2) external onlySender {
        holderRewardCondition = amount;
        NFTRewardCondition = amount1;
        condition = amount2;
    }

    function setExcludeHolder(address addr, bool enable, bool enableNFT) external onlySender {
    ...
    function addNFT(address[] calldata adrs) public onlySender{
        for(uint i=0;i<adrs.length;i++)
            addNFTHolder(adrs[i]);
    }
    ...
    function setAddress(address nft) public onlySender{
        NftAddress = nft;
    }

    function setMinTime(uint256 time1, uint256 time2) public onlySender{
        minRewardTime = time1;
...
contract BOSS is AbsToken {
    constructor(address f) AbsToken(
        ...
        18,
        1000000,
        address(0x2a45324462DBefEB99a597B0DcBF78742153FCAC),
        address(0x95Db1E88b122baBB3407840D93927Ee738193867)
    ){}
}