...
contract HKCToken is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(lpAddr == from || lpAddr == to){
            ...
            if(lpAddr == from){
                realVol = amount.mul(99).div(100);
                _baseTransfer(from,address(this),amount.mul(1).div(100));
    ...
    function setExcLock(bool _excLock) public onlyManager {
        excLock = _excLock;
    }

    function setSwap(bool _swapByMin,uint256 _swapMinVol) public onlyManager {
