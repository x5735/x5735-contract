...
// File: localhost/token/interface/IERC20.sol
...
// File: localhost/token/interface/IYouSwapFactory.sol
...
// File: localhost/token/interface/IYouSwapRouter.sol
...
// File: localhost/token/interface/IMoonProxy.sol
...
interface IMoonProxy {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) external returns (bool);

    function createPair(address creator, address router, address tokenMoon, address tokenB) external returns (address);
}
...
contract MoonProxy is IMoonProxy, InitializableOwnable {
    ...
    function youSwapPair() override external view returns (address){
        return _youSwapPair;
    }
    ...
    function createPair(address creator, address router, address tokenMoon, address tokenB) override external returns (address){
        ...
        IYouSwapRouter _youSwapRouter = IYouSwapRouter(router);
        // Create a YouSwap pair for this new token
        _youSwapPair = IYouSwapFactory(_youSwapRouter.factory())
        .createPair(tokenMoon, tokenB);

        // set the rest of the contract variables
        youSwapRouter = _youSwapRouter;
        _pairCreated = true;

        return _youSwapPair;
    }

    function swapAndLiquify(uint256 contractTokenBalance) onlyMoon lockTheSwap override external returns (bool)  {
...
contract MoonERC20Template is IERC20, InitializableOwnable {
    using SafeMath for uint256;
    ...
    IERC20 public constant YOU = IERC20(0xc697abbd732F2713ecd143ab95815396159a7F49);
    ...
    function init(
        ...
        _maxTxAmount = initSupply.div(200);
        ...
        address pair = moonProxy.createPair(creator, router, address(this), address(YOU));
