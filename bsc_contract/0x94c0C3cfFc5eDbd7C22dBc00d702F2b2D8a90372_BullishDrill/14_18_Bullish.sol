...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	function _mint_rewards(uint256 _pool_id) internal
	{
		...
		if(_pool.alloc_point == 0 || _pool.total_staked_amount == 0 ||
			block.number <= _pool.last_reward_block_id)
			return;
	...
	function _collect_reward(uint256 _pool_id, address _address_user) private returns(uint256)
	{
		...
		uint256 amount = get_user_locked_reward_amount(_pool_id, _address_user);
		if(amount > 0)
		{
	...
	function get_user_locked_reward_amount(uint256 _pool_id, address _address_user) public view returns(uint256)
	{
		PoolInfo memory _pool = pool_info[_pool_id];
		UserInfo memory _user = user_info[_pool_id][_address_user];
		...
		uint256 accu_user_reward_amount = user_boosted_staked_amount * accu_rps;
		uint256 locked_reward_amount = accu_user_reward_amount - _user.paid_reward_amount;
		
		return locked_reward_amount;
	}
