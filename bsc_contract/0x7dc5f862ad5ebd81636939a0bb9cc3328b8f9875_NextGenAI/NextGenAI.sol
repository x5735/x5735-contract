...
contract NextGenAI is ERC20Burnable, Ownable {
    using SafeMath for uint256;
    using Address for address payable;

    uint256 public buyFee = 0; // 0% buy fee
    uint256 public sellFee = 0; // 0% sell fee
    uint256 public constant MAX_FEE = 10; // Max fee is 10%
    address public uniswapV2Pair;

    constructor() ERC20("NextGenAI", "NXGN") {
        uint256 totalSupply = 5000000000 * 10**decimals();
    ...
    function setUniswapV2Pair(address pair) public onlyOwner {
        require(pair != address(0), "Pair address cannot be zero");
        uniswapV2Pair = pair;
    }
