...
contract Zc is Ownable {
    ...
    uint64 public constant ISSUE_PERIOD = 10 minutes; 
    ...
    constructor(
        ...
        Pool memory p = Pool(cap, 1679925600, 1, false, cap, 0, 0);
        pools.push(p);
        emit NewPool(0, cap, 1679925600);
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        require(balance >= amount, "balance not enough");
        require(balance - amount >= unwithdraw, "exceeds available");
        updateLast(msg.sender, stakingAmount, lastUpdate);
        ...
        for (uint256 poolId = 0; poolId < pools.length; ++poolId) {
            ...
            if (actualIssue >= 9) {
                for (
                    ...
                    uint256 a = amountByIssue(poolId, i) * INTEREST_MARGIN_RATE / 10000;
