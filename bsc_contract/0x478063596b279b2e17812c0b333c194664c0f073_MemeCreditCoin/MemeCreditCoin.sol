...
contract MemeCreditCoin is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    MemeCreditCoinDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    uint256 public _totalSupply = 999_999_999_999 * (10**18);
    ...
    address payable public _marketingWalletAddress = payable(0x6237eb68B0aC012a1A784E304445e4748A2390F5);
    address payable public _buybackWalletAddress = payable(0x174DB9a615a95E78267212E31992C1798b8A6c28);
    ...
    constructor() BEP20("Meme Credit Coin", "MCC") 
    {
        dividendTracker = new MemeCreditCoinDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "MemeCreditCoin: The dividend tracker already has that address");
        MemeCreditCoinDividendTracker newDividendTracker = MemeCreditCoinDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "MemeCreditCoin: The new dividend tracker must be owned by the MemeCreditCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MemeCreditCoin: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MemeCreditCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MemeCreditCoin: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MemeCreditCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MemeCreditCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MemeCreditCoin: Cannot update gasForProcessing to same value");
...
contract MemeCreditCoinDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MemeCreditCoin_Dividend_Tracker", "MemeCreditCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MemeCreditCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MemeCreditCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MemeCreditCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MemeCreditCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MemeCreditCoin_Dividend_Tracker: Cannot update claimWait to same value");
