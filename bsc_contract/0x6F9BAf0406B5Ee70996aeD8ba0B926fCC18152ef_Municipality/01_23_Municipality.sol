  ...
  function getBundles(address _user) external view returns (uint256[6][17] memory) {
    ...
    function _updateAdditionLevel(address _user) private {
        ...
        for (uint8 i = 0; i < 4; i++) {
            ...
            if (userToPurchasedAmountMapping[_user] >= 11322 * 1e18){// sb[3]) {
                _additionalLevel = 21;
            } else if (userToPurchasedAmountMapping[_user] >= 5466 * 1e18){//sb[2]) {
                _additionalLevel = 14;
            } else if (userToPurchasedAmountMapping[_user] >= 2938 * 1e18){//sb[1]) {
                _additionalLevel = 9;
            } else if (userToPurchasedAmountMapping[_user] >= 618 * 1e18){// sb[0]) {
                _additionalLevel = 5;
            }  
