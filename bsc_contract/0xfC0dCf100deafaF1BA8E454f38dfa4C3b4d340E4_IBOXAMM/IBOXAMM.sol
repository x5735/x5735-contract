...
contract IBOXAMM {
    ...
    address public admin = 0x13fecF46dB227D85b843E37211112E00Aa55d33B;
   ...
   struct UserInfo {  
    ...
    constructor () {
        owner =  msg.sender;
        AGPAIR.approve(address(uniswapV2Router),MAX_INT);
    ...
    function buyInsurance(uint256 insuranceId, uint256 _amount) public nonReentrant() {
         ...
         swapAndLiquifyAG(_amount,address(this));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant() {
        ...
        if (_pid == 4) {
            uint256 initialBalance = IERC20(AGPAIR).balanceOf(address(this));
            swapAndLiquifyAG(_amount/2,address(this));
            uint256 AGLPBalance = IERC20(AGPAIR).balanceOf(address(this));
            uint256 newLpBalance = AGLPBalance.sub(initialBalance);
    ...
    function withdraw(uint256 orderId) public nonReentrant() {
        ...
        user.lirun+=count;
        if (user.PoolId == 4) {
             swapAndLiquifyAG(usdtAways,user.userAddress);
        }else {
    ...
    function emergencyWithdraw(uint256 orderId) public nonReentrant() {
         ...
         if (user.PoolId == 4) {
            ...
            uniswapV2Router.removeLiquidity(address(AG),address(USDT),user.LPAMOUNT,0,0,address(this),block.timestamp); 
            uint256 IAGBalance = IERC20(AG).balanceOf(address(this));
            uint256 newBalance = IAGBalance.sub(initialBalance);
            swapTokensForTokens(newBalance,address(this),address(AG),address(USDT));
         }else {
    ...
    function adminSetdeposit(address userAddr,uint256 _amount, uint256 _lastRewardBlock, uint256 _CountMoney, uint256 _newLingQu, uint256 _CountLirun , uint256 _startTime, uint256 _endTime, uint256 _LPAMOUNT, uint256 _pid,uint256 _LastSubordinate) public onlyOwner() {
            ORDERID++;
            userOrdersIds[userAddr].push(ORDERID);
            UserInfo storage user = userOrders[ORDERID];
            user.orderId = ORDERID;
            user.userAddress = userAddr;
            user.amount = _amount;
            user.lirun = 0;
            user.lastRewardBlock = _lastRewardBlock;
            user.CountMoney = _CountMoney ; 
            user.newLingQu = _newLingQu;
            user.CountLirun = _CountLirun; 
            user.startTime = _startTime;
            user.endTime = _endTime;
            user.PoolId = _pid;
            user.LastSubordinate =  _LastSubordinate;
            user.LPAMOUNT = _LPAMOUNT;
            userAllOrd[userAddr].push(user);
    }
