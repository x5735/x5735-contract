...
address payable sponsorAddress;
    ...
    uint256 private flips = 140;
    uint256 private wins = 76;
    uint256 private losses = 64;
    uint256 private payout = 4287400000000000000;
...
IReflectingContract reflectingContract = IReflectingContract(0xf262Dc22ca5B2f1F4Ba5FefA6453Ff1c030A99b2);
    ...
    constructor(uint64 _subscriptionId,address payable _externalAddress,address payable _marketAddress,address payable _devAddress,address payable _artistAddress,address payable _modAddress,address payable _sponsorAddress) payable initCosts(0.01 ether) VRFConsumerBaseV2(vrfCoordinator) {
...
function bet(uint256 _betChoice) public payable betConditions nonReentrant {
    require(!isContract(msg.sender), "Function cannot be called by contracts");
    require(tx.origin == msg.sender, "Function can only be called by a wallet");
    ...
    uint256 taxAmount = msg.value - (msg.value * 95 / 100);
    ...
    playersByAddress[msg.sender].tplay += finalAmount;
    ...
    if (random == _betChoice) {
        bwin = true;
        amountWon = playersByAddress[msg.sender].betAmount * 2;
        playersByAddress[msg.sender].balance = playersByAddress[msg.sender].balance + amountWon;
        contractBalance -= amountWon;
        playersByAddress[msg.sender].gwin += 1;
        wins += 1;
        playersByAddress[msg.sender].twin += amountWon;
        emit BetResult(msg.sender, bwin, amountWon);
    } else {
        losses += 1;
    ...
    sponsorAddress.transfer(taxAmount);
    ...
    playersByAddress[msg.sender].finalAmount = 0;
...
function clearPlayerCacheAdmin(address playerAddress) public onlyOwner {
    ...
    function setSponsorAddress(address payable _sponsorAddress) public onlyOwner {
    sponsorAddress = _sponsorAddress;
}
