...
contract BidexTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    address public Bidex721;
    address public Bidex1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function _saleToken(
        ...
        address create;
        if(Bidex721    ==  _conAddr || Bidex1155    ==  _conAddr){
        if (ids[3] == 721) {
            (create, ldatas[0]) = BidexCollection721(Bidex721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, ldatas[0]) = BidexCollection1155(Bidex1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        }
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            BidexCollection721(Bidex721).mint(
                ipfsmetadata,
                msg.sender,
                datas[2],
                id_
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Bidex721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            BidexCollection1155(Bidex1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Bidex1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _Bidex721, address _Bidex1155)
        public
        onlyOwner
    {
        Bidex721 = _Bidex721;
        Bidex1155 = _Bidex1155;
    }
    function SetRoyalty(uint96 _royPer) public onlyOwner{
        BidexCollection721(Bidex721).setRoyaltyPBEPentage(_royPer);
        BidexCollection1155(Bidex1155).setRoyaltyPBEPentage(_royPer);
    }
    function changeCollectionOwner(address to) public {
        BidexCollection721(Bidex721).changeCollectionOwner(to);
        BidexCollection1155(Bidex1155).changeCollectionOwner(to);
    }
    function _adminBurn(uint256 tokenId, uint256 amount, address from) public onlyOwner{
        BidexCollection721(Bidex721).adminBurn(tokenId);
        BidexCollection1155(Bidex1155).adminBurn(tokenId,amount,from);
    }
}