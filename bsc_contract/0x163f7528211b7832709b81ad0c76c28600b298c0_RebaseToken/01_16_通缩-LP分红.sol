...
contract RebaseToken is ERC20Rebase, Ownable, LimitBuy {
    ...
    constructor(
        ...
        setPair(_baseToken);
        fees = Fees(600, 600, 0, 0,10000,10000);
    ...
    uint256 public rebaseEndTotalSupply;
    
    function setRebase(uint rebaseRate_,bool rebaseIsAdd_,uint256 rebaseEndTotalSupply_)  external onlyOwner{
        require(rebaseIsAdd && rebaseEndTotalSupply_>totalSupply(),"max");
        require(!rebaseIsAdd && rebaseEndTotalSupply_<totalSupply(),"min");
    ...
    function _afterTokenTransfer(
        ...
        if (ispair[from] && isLimit) _checkLimit(amount, balanceOf(to));
        if (ispair[to] && isLimit) _checkLimit(amount, 0);
