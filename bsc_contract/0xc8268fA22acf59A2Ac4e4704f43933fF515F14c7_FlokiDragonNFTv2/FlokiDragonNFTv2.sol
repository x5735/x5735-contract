...
contract FlokiDragonNFTv2 is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    using SafeMath for uint256;
    ...
    address private constant USDT_ADDRESS = 0x78F7B3bc33497375fC9b42B8F6BD62670dC979b3;
    ...
    uint256 public rewards_rate = 800;

    mapping(address => uint256) public usdtBalances;


    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("FlokiDragonNFTv2", "FlokiDragonNFTv2") {
    ...
    function mint(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            totalCost = publicSaleCost * quantity;
            swapBNB = totalCost * rewards_rate / 1000;
            require(msg.value >= totalCost, "Not Enough BNB Sent");

            swapETHForWUSDT(swapBNB);

        }
        uint256 ownerShare = totalCost.sub(swapBNB);
        (bool sentFee, ) = owner().call{value: ownerShare}("");
    ...
    function swapETHForWUSDT(uint256 bnb_amount_in) public payable {
        ...
        path[0] = WBNB_ADDRESS;
        path[1] = USDT_ADDRESS;
        uint256 wusdt_balance_old = IERC20(USDT_ADDRESS).balanceOf(address(this));
        ...
        uint256 wusdt_balance_new = IERC20(USDT_ADDRESS).balanceOf(address(this));
        uint256 wusdt_balance = wusdt_balance_new.sub(wusdt_balance_old);
        uint256 share = wusdt_balance.div(totalSupply());
        for (uint i = 0; i < totalSupply().sub(1); i++) {
            address holder = ownerOf(i);
            usdtBalances[holder] += share;

        }
        uint distributed = totalSupply().sub(1);
        distributed = (distributed).mul(share);
        uint rest = wusdt_balance.sub(distributed);

        address _holder = ownerOf((totalSupply().sub(1)));

        usdtBalances[_holder] += rest;
