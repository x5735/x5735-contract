...
contract RandomNumberGame is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    uint256 public cPrize;

    uint256 constant MAX_WINNER_HISTORY = 10;
    WinnerInfo[MAX_WINNER_HISTORY] public winnersHistory;
    uint256 public winnerHistoryIndex;

    event NewRound(uint256 round);
    event WinnerSelected(uint256 round, address winner, uint256 prize);

    event PlayerJoined(address indexed player, uint256 amount);

    constructor(address payable _marketing, address payable _project) VRFConsumerBaseV2(0xc587d9053cd1118f25F645F9E08BB98c9712A4EE) ConfirmedOwner(msg.sender) {
        marketing = _marketing;
        project = _project;
        COORDINATOR = VRFCoordinatorV2Interface(0xc587d9053cd1118f25F645F9E08BB98c9712A4EE);
    }

    // VRFConsumer functions
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override nonReentrant {
        ...
        uint256 prize = ENTRY_FEE * MAX_PLAYERS;
        uint256 winnerPrize = (prize * 90) / 100;
        uint256 projectFee = (prize * 3) / 100;
        uint256 marketingFee = (prize * 7) / 100;
        ...
        emit WinnerSelected(currentRound - 1, winner, winnerPrize);

        winnersHistory[winnerHistoryIndex] = WinnerInfo({
        roundId: currentRound - 1,
        winnerAddress: winner,
        prizeWon: winnerPrize
        });
        winnerHistoryIndex = (winnerHistoryIndex + 1) % MAX_WINNER_HISTORY;
    ...
    function join() external payable nonReentrant {
        ...
        cPrize = ENTRY_FEE * players.length;
    ...
    function setProjectAddress(address payable _project) external onlyOwner {
        project = _project;
    }
    ...
    function getRecentWinners() external view returns (WinnerInfo[MAX_WINNER_HISTORY] memory) {
