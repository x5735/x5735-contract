...
contract ATE is ERC20 {
    ...
    function _transfer(
		...
		bool isperparInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparinviter[to] == address(0) && amount >= 10**2;
		...
		if(isperparInviter){
			...
			bool isInviter = inviter[to] == address(0) && perparinviter[from] == to;
			if(isInviter){
				inviter[to] = from;
			}
		}
	...
	function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 5; i++) {
			...
			_mint(recieveD, amount.div(100).mul(rate));
        }
    }
}