...
contract SwapX2YModule {
    ...
    /// Delegate call implementation for IBiswapPoolV3#swapX2Y.
    function swapX2Y(
        address recipient,
        uint128 amount,
        int24 lowPt,
        ...
        while (lowPt <= st.currentPoint && !cache.finished) {
            ...
            if (cache.currentOrderOrEndpt & 2 > 0) {
                ...
                if (amountNoFee > 0) {
                    ...
                    if (costX >= amountNoFee) {
                        ...
                        feeAmount = uint128(uint256(costX) * _fee / (1e6 - _fee));
                        uint256 mod = uint256(costX) * _fee % (1e6 - _fee);
                        if (mod > 0) {
                            feeAmount += 1;
                        }
                    }
                ...
                if (amountNoFee > 0) {
                    if (st.liquidity > 0) {
                        ...
                        if (retState.costX >= amountNoFee) {
                            ...
                            feeAmount = uint128(uint256(retState.costX) * _fee / (1e6 - _fee));
                            uint256 mod = uint256(retState.costX) * _fee % (1e6 - _fee);
                            if (mod > 0) {
                                feeAmount += 1;
                            }
                        }
                    ...
                    if (!cache.finished) {
                        ...
                        st.feeTimesL = pointdata.liquidDelta < 0 ? st.feeTimesL + pointdata.feeTimesL : st.feeTimesL - pointdata.feeTimesL;
            ...
            if (st.liquidity == 0) {
                ...
                uint128 amountNoFee = uint128(uint256(amount) * (1e6 - st.fee) / 1e6);
                if (amountNoFee > 0) {
                    SwapMathX2Y.RangeRetState memory retState = SwapMathX2Y.x2YRange(
                    ...
                    if (retState.costX >= amountNoFee) {
                        ...
                        feeAmount = uint128(uint256(retState.costX) * st.fee / (1e6 - st.fee));
                        uint256 mod = uint256(retState.costX) * st.fee % (1e6 - st.fee);
                        if (mod > 0) {
                            feeAmount += 1;
                        }
                    }
        ...
        uint256 bx = balanceX();
        IBiswapCallback(msg.sender).swapX2YCallback(amountX, amountY, data);
        require(balanceX() >= bx + amountX, "XE");

    }

    /// Delegate call implementation for IBiswapPoolV3#swapX2YDesireY.
    function swapX2YDesireY(
        ...
        while (lowPt <= st.currentPoint && !cache.finished) {
            ...
            if (cache.currentOrderOrEndpt & 2 > 0) {
                ...
                uint256 feeAmount = MulDivMath.mulDivCeil(costX, _fee, 1e6 - _fee);
                totalFeeXCharged += feeAmount;
            ...
            if (cache.currentOrderOrEndpt & 1 > 0) {
                if (st.liquidity > 0) {
                    ...
                    uint256 feeAmount = MulDivMath.mulDivCeil(retState.costX, _fee, 1e6 - _fee);
...
//            int24 nextVal = orderOrEndpoint.getOrderOrEndptVal(nextPt, cache.pointDelta); //stack to deep
            ...
            if (st.liquidity == 0) {
                ...
                st.sqrtPrice_96 = LogPowMath.getSqrtPrice(st.currentPoint);
                // st.liquidityX must be 0
                cache.currentOrderOrEndpt = orderOrEndpoint.getOrderOrEndptVal(nextPt, cache.pointDelta);
            } else {
                // amount > 0
                SwapMathX2YDesire.RangeRetState memory retState = SwapMathX2YDesire.x2YRange(
