...
abstract contract AbcToken is ERC20, Ownable {
    ...
    uint256 private tokensSwapForDividend = 200 * 10**18;
    ...
    function _transfer(
        ...
        if (!_feeList[from] && !_feeList[to]) {
            if (_swapPairList[from] || _swapPairList[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap && !isAddLdx) {
                        if (balanceOf(address(this)) > tokensSwapForDividend) {
                            swapToken(tokensSwapForDividend, address(warp));
                            warp.withdraw();
                        }
                    }
    ...
    function addLPHolder(address adr) private {
        ...
        if (0 == holderIndex[adr]) {
            if (0 == lpHolders.length || lpHolders[0] != adr) {
                holderIndex[adr] = lpHolders.length;
                lpHolders.push(adr);
            }
        }
    }
    ...
    function allLpHolders() external view returns (address[] memory) {
        return lpHolders;
    }
...
contract VersusDao is AbcToken {
    constructor()
        AbcToken(
            address(0x10ED43C718714eb63d5aA57B78B54704E256024E), //RouterAddress
            address(0x55d398326f99059fF775485246999027B3197955), //BaseToke
            "Versus Dao", //Name
            "VSD", //Symbol
            10000 * 10**18, //Supply
            address(0xE29BB0BF845c895566934b06DFEc74A5e0C20096) //LPAddress
        )
    {}
}