...
contract Arbitroge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ArbitrogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Arbitroge", "AOGE") {
        marketingWallet = payable(0x73F6cbA5cBb0789988B570633D1e3C430d5135b1);
        devWallet = payable(0x3b89EF4C89F51a5649E7BA803C21B290E0C3DF11);
        ...
        dividendTracker = new ArbitrogeDividendTracker(
            payable(this),
            router,
...
contract ArbitrogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Arbitroge public ArbitrogeContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ArbitrogeContract = Arbitroge(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Arbitroge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Arbitroge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Arbitroge contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ArbitrogeContract);

            uint256 prevBalance = ArbitrogeContract.balanceOf(address(this));
            ...
            {
                uint256 received = ArbitrogeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ArbitrogeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
