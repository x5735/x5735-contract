...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    mapping(uint256 => bool) public lockDisabled;  
    ...
    function buySmartLPforToken(uint256 amount) external {
        require(amount >= minPurchaseAmountToken, 'Token price is more than sent');
    ...
    function burnSmartLP(uint tokenId) external nonReentrant {
        ...
        require(lockDisabled[tokenId] || block.timestamp > userSupply.SupplyTime + lockTime, 
            "Token is locked"
        );
    ...
    function updatePaymentToken(address _paymentToken) external onlyOwner {
        require(AddressUpgradeable.isContract(_paymentToken), "Not a contract");
        paymentToken = IERC20Upgradeable(_paymentToken);
    }

    function updateSwapRouter(address newSwapRouter, address newPancakeRouter) external onlyOwner {
        require(AddressUpgradeable.isContract(newSwapRouter), "Not a contract");
    ...
    function updateNimbTokenContract(address _nimbToken, address _NimbBnbPair, address newLpStaking) external onlyOwner {
        require(AddressUpgradeable.isContract(_nimbToken) && AddressUpgradeable.isContract(_NimbBnbPair) && AddressUpgradeable.isContract(newLpStaking), "Not a contract");
    ...
    function updateGnimbTokenContract(address _gnimbToken, address _GnimbBnbPair,address newLpStaking) external onlyOwner {
        require(AddressUpgradeable.isContract(_gnimbToken) && AddressUpgradeable.isContract(_GnimbBnbPair) && AddressUpgradeable.isContract(newLpStaking), "Not a contract");
    ...
    function updateLendingContract(address _newLendingContract) external onlyOwner {
        require(AddressUpgradeable.isContract(_newLendingContract), "Not a contract");
    ...
    function updateTokenAllowance(address token, address spender, int amount) external onlyOwner {
        require(AddressUpgradeable.isContract(token), "Not a contract");
        uint allowance;
        if (amount < 0) {
    ...
    function updatePurchaseToken (address _purchaseToken) external onlyOwner {
        require(AddressUpgradeable.isContract(_purchaseToken), "Not a contract");
        purchaseToken = IERC20Upgradeable(_purchaseToken);
