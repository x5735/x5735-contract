...
contract AI is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("AI", "11AI") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
                ...
                if(startTime.add(300) > block.timestamp){require(balanceOf(to).add(amount) <= 25 * 10**26,"max amount");}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
				...
				hoderAmount = hoderAmount.add(amount.div(100).mul(1));
    ...
    function swapAndLiquify() private {
		...
		if(thisAmount > hoderSwap.add(10**19) && hoderSwap > 10**19){
