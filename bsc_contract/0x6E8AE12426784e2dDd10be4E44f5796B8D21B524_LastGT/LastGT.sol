...
contract LastGT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LastGTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LastGT", "GT") {
        marketingWallet = payable(0x7299336E094dd0f5a74f6bdCbfE7fECc401b81C4);
        missionsWallet = payable(0x7299336E094dd0f5a74f6bdCbfE7fECc401b81C4);
        lmsBurnWallet = payable(0x7299336E094dd0f5a74f6bdCbfE7fECc401b81C4);
        ...
        dividendTracker = new LastGTDividendTracker(payable(this), router, 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
                                                   "LastGTTRACKER", "GTTRACKER");
...
contract LastGTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        LastGTContract = LastGT(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main LastGT contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LastGTContract);

            uint256 prevBalance = LastGTContract.balanceOf(address(this));
            ...
            {
                uint256 received = LastGTContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LastGTContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
