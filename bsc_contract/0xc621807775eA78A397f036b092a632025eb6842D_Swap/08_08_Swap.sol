...
contract Swap is Base, IBiswapCallback {
    ...
    function swapY2XCallback(
        ...
        (address token0, address token1, uint16 fee) = dt.path.decodeFirstPool();
    ...
    function swapDesireInternal(
        ...
        if (tokenOut < tokenIn) {
            ...
            (acquire, ) = IBiswapPoolV3(poolAddr).swapY2XDesireX(
            ...
            // tokenOut is tokenY
            // tokenIn is tokenX
            (, acquire) = IBiswapPoolV3(poolAddr).swapX2YDesireY(
                recipient, uint128(desire), -800001,
                abi.encode(data)
            );
        }
    ...
    function swapAmountInternal(
        ...
        while (true) {
            ...
            if (tokenIn < tokenOut) {
                // swapX2Y
                uint256 costX;
                (costX, acquire) = IBiswapPoolV3(poolAddr).swapX2Y(
                ...
                // swapY2X
                uint256 costY;
                (acquire, costY) = IBiswapPoolV3(poolAddr).swapY2X(
                    hasMultiplePools? address(this): recipient, amount, 799999,
                    abi.encode(SwapCallbackData({path: abi.encodePacked(tokenIn, fee, tokenOut), payer: payer}))
                );
                if (firstHop) {
    ...
    function swapY2X(
        ...
        (uint256 amountX, ) = IBiswapPoolV3(poolAddr).swapY2X(
    ...
    function swapY2XDesireX(
        ...
        (amount.amountX, amount.amountY) = IBiswapPoolV3(poolAddr).swapY2XDesireX(
            recipient, swapParams.amount, swapParams.boundaryPt,
    ...
    function swapX2Y(
        ...
        address payer = msg.sender;
        address recipient = (swapParams.recipient == address(0)) ? address(this): swapParams.recipient;
        (, uint256 amountY) = IBiswapPoolV3(poolAddr).swapX2Y(
            recipient, swapParams.amount, swapParams.boundaryPt,
            abi.encode(SwapCallbackData({path: abi.encodePacked(swapParams.tokenX, swapParams.fee, swapParams.tokenY), payer: payer}))
        );
    ...
    function swapX2YDesireY(
        ...
        address recipient = (swapParams.recipient == address(0)) ? address(this): swapParams.recipient;
        ExchangeAmount memory amount;
        (amount.amountX, amount.amountY) = IBiswapPoolV3(poolAddr).swapX2YDesireY(
            recipient, swapParams.amount, swapParams.boundaryPt,
            abi.encode(SwapCallbackData({path: abi.encodePacked(swapParams.tokenY, swapParams.fee, swapParams.tokenX), payer: payer}))
        );
