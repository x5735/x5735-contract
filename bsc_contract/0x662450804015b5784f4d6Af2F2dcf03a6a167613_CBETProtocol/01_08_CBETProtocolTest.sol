...
contract CBETProtocol is Ownable, IERC20Metadata {
    ...
    string private _name = "OKTEST";
    string private _symbol = "OKTEST";
    ...
    uint256 firstIssueTime = 1679632200;
    ...
    function _transfer(
        ...
        if (from == address(uniswapV2PairUSDT)) {
            ...
            if(_lastRebasedTime > 0 && block.timestamp < (startTime + 6 hours)){
                ...
                require(get24HourTransfers(to) + getPrice(usdt,amount) < (98 * 10 ** _decimals - 1), "Exceeding the purchase limit");
                // 本次交易写入钱包地址的买入交易记录
    ...
    function _fee(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if (from == address(uniswapV2PairUSDT)){
            ...
            for (uint256 i = 0; i < 5; i++) {
                ...
                if (usdtPair.balanceOf(nowAddress) * IUsdt.balanceOf(address(uniswapV2PairUSDT)) / usdtPair.totalSupply() < (98 * 10 ** (_decimals - 2))) {
                    _basicTransfer(from, DEAD, allFee);
                    break;
                }
                // 大于18.8USDT，正常发放
