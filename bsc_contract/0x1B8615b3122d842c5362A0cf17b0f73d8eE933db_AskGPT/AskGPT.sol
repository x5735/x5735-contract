...
contract AskGPT is IERC20, Ownable {
    using Address for address;
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "AskGPT";
    string constant _symbol = "AGPT";
    uint8 constant _decimals = 18;

    uint256 constant _totalSupply = 1_000_000_000 * (10 ** _decimals);


    uint256 public _maxTxAmount = _totalSupply ; //~0.125%
    uint256 public _maxWalletSize = _totalSupply ; //0.25%
    ...
    uint256 antiDumpPeriod = 0 minutes;
    ...
    function setSellPeriod(uint256 _sellPercentIncrease, uint256 _period) external onlyOwner {
        require((totalFee * _sellPercentIncrease) / 100 <= 1000, "Sell tax too high");
    ...
    function setAntiDumpTax(uint256 _tax, uint256 _period, uint256 _threshold, bool _reserve0) external onlyOwner {
        require(_threshold >= 10 && _tax <= 1000 && (_tax == 0 || _tax >= sellPercent) && _period <= 1 hours, "Parameters out of bounds");
        antiDumpTax = _tax;
    ...
    function setFees(uint256 _liquidityFee, uint256 _giveawayFee, uint256 _devmarketingFee, uint256 _sellBias, uint256 _feeDenominator) external onlyOwner {
        ...
        require(totalFee <= feeDenominator , "Fees too high");
