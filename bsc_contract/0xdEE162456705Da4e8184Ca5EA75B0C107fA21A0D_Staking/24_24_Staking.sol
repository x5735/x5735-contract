...
contract Staking is IStaking, Ownable, ERC721Holder {
     ...
     */
    function getStakeApyForTier(uint32 _nftTier) public view override returns (uint8) {
     ...
     */
    function setStakeApyForTier(uint32 _nftTier, uint8 _apy) public override onlyOwner {
    ...
    function stake(
        ...
        uint8 baseApy = nftTierApys[HREANFT(nft).getNftTier(_nftIds[0])];
        bool isValidNftArray = true;
        for (uint index = 1; index < _nftIds.length; index++) {
            uint8 currentApy = nftTierApys[HREANFT(nft).getNftTier(_nftIds[index])];
    ...
    function calculateRewardInUsd(
        ...
        uint256 rewardInUsd = (_totalValueStakeUsd * _apy * yearAmount) / 1000;
        return rewardInUsd;
    }
