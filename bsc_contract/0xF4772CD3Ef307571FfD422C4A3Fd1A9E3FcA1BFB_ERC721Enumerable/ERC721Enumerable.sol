...
// File: contracts\ERC721AEnumerable.sol


pragma solidity ^0.8.0;
contract ERC721Enumerable is IERC721Metadata, IERC721Enumerable, Initializable{
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  uint256 internal constant maxBatchSize = 200;
  ...
  function _safeMint(address to, uint256 quantity, bytes memory _data, bool check) internal {
  ...
  function setTokenUri(string memory tokenUri_) external onlyGov{
      _tokenUri = tokenUri_;
  }

  function setAdmin(address _gov) external onlyGov{
      gov = _gov;
  }

  function setMinter(address minter, bool enable) external onlyGov{
  ...
  function mint(address to, uint256 quantity) external onlyMinter{
      _safeMint(to, quantity, "", false);
  }

  function safeMint(address to, uint256 quantity, bytes memory data) external onlyMinter{
      _safeMint(to, quantity, data, true);
  }
