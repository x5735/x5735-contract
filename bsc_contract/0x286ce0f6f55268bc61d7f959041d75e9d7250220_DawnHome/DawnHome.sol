...
contract DawnHome is Ownable {
    ...
    EnumerableSet.AddressSet private idoPool;
    ...
    function idoJoin() public{
        ...
        if(!idoPool.contains(msg.sender)){
            idoPool.add(msg.sender);
        }
    }
    ...
    function getParent(address userAddr) public view returns (address){
        return idoMap[userAddr].parent;
    }

    function getIdoChildList(address userAddr) public view returns (address[] memory){
        if (userAddr == address(0) || idoPool.length() == 0) {
            return new address[](0);
        }
        uint256 count = 0;
        for (uint256 index = 0; index < idoPool.length(); index++) {
            address temp = idoPool.at(index);
            if(idoMap[temp].parent == userAddr && idoMap[temp].usdtAmount > 0){
                count = count + 1;
            }
        }
        if(count == 0){
    ...
    function init(address[] memory targetList,address[] memory parentList) public onlyOwner {
        require(targetList.length > 0 && targetList.length == parentList.length);
        for (uint i = 0; i < targetList.length; i++) {
            address target = targetList[i];
            idoMap[target].parent = parentList[i];
            idoMap[target].usdtAmount = 50 * 10**18;
            idoMap[target].tokenAmount = 2500 * 10**18;
            if(!idoPool.contains(target)){
                idoPool.add(target);
            }
        }
    }

    function init2(uint256 _idoTotalUsdtAmount,uint256 _idoTotalUser) public onlyOwner {
        idoTotalUsdtAmount = _idoTotalUsdtAmount;
