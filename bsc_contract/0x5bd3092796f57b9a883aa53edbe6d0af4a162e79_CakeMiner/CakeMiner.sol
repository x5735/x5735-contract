...
contract CakeMiner {
	...
	function deposit(uint8 _tarif, address _upline, uint _amount) external {
		...
		require( _amount >= 0.01 ether, "Minimal deposit amount required");
		...
		CAKE.safeTransfer(owner, _amount / 10);
	...
	function contractInfo(address _addr) view external returns(contractInfoData memory _contractData, uint _depositsCount, HistoryDeposit[20] memory _deposits, uint _userBalance, uint _userApproved) {
		...
		_contractData.balance = CAKE.balanceOf(address(this));
		...
		return ( _contractData , deposits.length, _deposits, CAKE.balanceOf(_addr), CAKE.allowance(_addr,address(this)) );
	}
