...
contract LastManGiving is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LastManGivingDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public missionsWallet;
    address payable public lmsBurnWallet;
    ...
    uint256 public buyMissionsFee;
    uint256 public sellMissionsFee;
    uint256 public buyLMSFee;
    uint256 public sellLMSFee;
    ...
    event updateMissionsWallet(address wallet);
    event updateLMSBurnWallet(address wallet);            
    ...
    event UpdateFees(uint256 sellMarketingFees, uint256 sellLiquidityFee, uint256 sellRewardsFee,
                     ...
                     uint256 buyMissionsFee, uint256 sellMissionsFee, uint256 buyLMSFee, uint256 sellLMSFee);
    ...
    constructor() ERC20("LastManGiving", "LMG") {
        marketingWallet = payable(0x102EE4DdeB7A95Ef9aaAecc14AB93363E3D9B5d5);
        missionsWallet = payable(0xf6dC7600408F05FCB05795261A9C50a8b13a77A2);
        lmsBurnWallet = payable(0xBCCDe54A363daD75C481Cbfc538c12A75ACd0982);
        ...
        totalBuyFees = buyRewardsFee.add(buyLiquidityFee).add(buyMarketingFees).add(buyMissionsFee).add(buyLMSFee);
        totalSellFees = sellRewardsFee.add(sellLiquidityFee).add(sellMarketingFees).add(sellMissionsFee).add(sellLMSFee);

        dividendTracker = new LastManGivingDividendTracker(payable(this), router, 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
                                                   "LastManGivingTRACKER", "LMGTRACKER");
        ...
        _isExcludedFromFees[address(missionsWallet)] = true;
        _isExcludedFromFees[address(lmsBurnWallet)] = true;
        _isExcludedFromFees[msg.sender] = true;

        uint256 totalTokenSupply = (7_000_000) * (10**18);
    ...
    function setMissionsWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        missionsWallet = payable(wallet);
        emit updateMissionsWallet(wallet);
    }        
    
    function setLMSBurnWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        lmsBurnWallet = payable(wallet);
        emit updateLMSBurnWallet(wallet);
    }           
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell,
                        ...
                        uint256 missionBuy, uint256 missionSell,
                        uint256 lmsBuy, uint256 lmsSell) public onlyOwner {
        ...
        buyMissionsFee = missionBuy;
        sellMissionsFee = missionSell;
        buyLMSFee = lmsBuy;
        sellLMSFee = lmsSell;
        ...
        emit UpdateFees(sellMarketingFees, sellLiquidityFee, sellRewardsFee,buyMarketingFees, 
                        buyLiquidityFee, buyRewardsFee, buyMissionsFee, sellMissionsFee, buyLMSFee, sellLMSFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            uint256 totalFees = RewardsFee.add(liquidityFee + marketingFees + missionFees + lmsFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .mul(buyRewardsFee).div(buyRewardsFee + buyMarketingFees + buyMissionsFee + buyLMSFee);
        }
        if (sellRewardsFee > 0) {
            dividendsFromSell = address(this).balance.mul(sellAmount).div(totalAmount)
            .mul(sellRewardsFee).div(sellRewardsFee + sellMarketingFees + sellMissionsFee + sellLMSFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellMissionsFee).add(sellLMSFee) +
            buyMarketingFees.add(buyMissionsFee).add(buyLMSFee);
        ...
        uint256 missionPayout = buyMissionsFee.add(sellMissionsFee) * feePortions;
        uint256 lmsPayout = buyLMSFee.add(sellLMSFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (missionPayout > 0) {
            (successOp2, ) = address(missionsWallet).call{value: missionPayout}("");
        }
        if (lmsPayout > 0) {
            (successOp3, ) = address(lmsBurnWallet).call{value: lmsPayout}("");
        }
        emit SendDividends(dividends, marketingPayout + missionPayout + lmsPayout, 
...
contract LastManGivingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LastManGiving public LastManGivingContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        LastManGivingContract = LastManGiving(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main LastManGiving contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LastManGivingContract);

            uint256 prevBalance = LastManGivingContract.balanceOf(address(this));
            ...
            {
                uint256 received = LastManGivingContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LastManGivingContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
