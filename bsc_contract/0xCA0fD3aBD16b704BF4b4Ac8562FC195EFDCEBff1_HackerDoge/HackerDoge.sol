...
contract HackerDoge is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('HackerDoge Token', 'HDT') {
        require(
            feeAccount != address(0),
            'HackerDoge: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'HackerDoge: incorrect fee percent');
        require(owner != address(0), 'HackerDoge: incorrect owner address');
        require(mintToAddr != address(0), 'HackerDoge: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'HackerDoge: transfer from the zero address');
        require(recipient != address(0), 'HackerDoge: transfer to the zero address');
