...
contract Coin_AU is ERC20 {

	address owner;
	event Mint (address indexed mintBy);
	uint256 public transferTaxAmt0 = uint256(50000);
	address public transferTaxAddress0 = address(0xCf5DF2F930be1e3f0615376d7e961a9555cad4A8);

	constructor() ERC20("AUniverse", "AU") {
		owner = msg.sender;
	}
 ...
 * calls super.transfer with variable receipient as receipient, variable amount as (((_amount) * ((1000000) - (50000) - (transferTaxAmt0))) / (1000000))
 * calls super._burn with variable account as (the address that called this function), variable amount as (((_amount) * (5)) / (100))
...
*/
	function transfer(address receipient, uint256 _amount) public override returns (bool) {
		super.transfer(receipient, ((_amount * (uint256(1000000) - uint256(50000) - transferTaxAmt0)) / uint256(1000000)));
		super._burn(msg.sender, ((_amount * uint256(5)) / uint256(100)));
 ...
 * checks that ((totalSupply) + (1000000000000000000000)) is less than or equals to 1000000000000000000000
 * checks that ((balanceOf with variable account as (the address that called this function)) + (1000000000000000000000)) is less than or equals to 2000000000000000000
...
*/
	function mint() public onlyOwner {
		require(((totalSupply() + uint256(1000000000000000000000)) <= uint256(1000000000000000000000)), "Exceeded Total Supply");
		require(((balanceOf(msg.sender) + uint256(1000000000000000000000)) <= uint256(2000000000000000000)), "Exceeded Individual Supply Constraints");
