...
contract Liquifier {
	...
	function _addLiquidity() private {
		...
		RouterInterface(router).addLiquidity(liquifiable, weth, _amtToken, _amtWETH, 0, 0, Bunny(liquifiable).team(), block.timestamp);
	}
...
contract Bunny is Owned {
	...
	uint256 public burnTax = 1;	// 1%
	uint256 public teamTax = 8;	// 8%
	uint256 public liqTax = 2;	// 2%
	...
	function _transfer(address from, address to, uint256 tokens) private {
		...
		if (!isLiquifying) {	// liquifying when to == liquifiablePair messes with reserves
			isLiquifying = true;	// lock variable to prevent infinite Ye
