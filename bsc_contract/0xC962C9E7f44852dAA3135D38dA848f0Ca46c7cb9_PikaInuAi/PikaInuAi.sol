...
contract PikaInuAi is BEP20 {
    IDEXRouter public router =
        IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    DividendDistributor public distributor;
    InterfaceLP public pairContract;
    //token data
    string public constant name = "Pika Inu V2";
    ...
    function update_fees() internal {
        require(totalBuyFee <= 100, "Buy tax cannot be more than 10%");
        require(totalSellFee <= 200, "Sell tax cannot be more than 20%");
        emit UpdateFee(
            uint8((totalBuyFee * 100) / 100),
            uint8((totalSellFee * 200) / 200)
        );
    }
