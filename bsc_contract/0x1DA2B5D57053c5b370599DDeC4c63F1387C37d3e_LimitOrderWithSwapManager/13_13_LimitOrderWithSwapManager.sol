...
contract LimitOrderWithSwapManager is Switch, Base, IBiswapAddLimOrderCallback, IBiswapCallback {
    ...
    function _addLimOrder(
        ...
        if (acquire > 0) {
            IBiswapPoolV3(pool).collectLimOrder(addLimitOrderParam.recipient, addLimitOrderParam.pt, 0, acquire, addLimitOrderParam.sellXEarnY);
        }
    }
        ...
        if (addLimitOrderParam.sellXEarnY) {
            if (addLimitOrderParam.pt < currentPoint) {
                ...
                if (addLimitOrderParam.isDesireMode) {
                    (costX, acquireY) = IBiswapPoolV3(pool).swapX2YDesireY(
                    ...
                    (costX, acquireY) = IBiswapPoolV3(pool).swapX2Y(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX,
                            fee: addLimitOrderParam.fee,
            ...
            if (addLimitOrderParam.pt > currentPoint) {
                ...
                uint256 acquireX;
                if (addLimitOrderParam.isDesireMode) {
                    (acquireX, costY) = IBiswapPoolV3(pool).swapY2XDesireX(
                    ...
                    (acquireX, costY) = IBiswapPoolV3(pool).swapY2X(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt + 1,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX,
                            fee: addLimitOrderParam.fee,
    ...
    function cancel(
        ...
        if (actualDecrease > 0) {
            if (sellXEarnY) {
                IBiswapPoolV3(pool).decLimOrderWithX(order.pt, actualDecrease);
            } else {
                IBiswapPoolV3(pool).decLimOrderWithY(order.pt, actualDecrease);
            }
        }
        if (actualDecrease > 0 || earn > 0) {
            IBiswapPoolV3(pool).collectLimOrder(recipient, order.pt, actualDecrease, earn, sellXEarnY);
        }
    ...
    function collect(
        ...
        if (earn > 0) {
            IBiswapPoolV3(pool).collectLimOrder(recipient, order.pt, 0, earn, sellXEarnY);
        }
