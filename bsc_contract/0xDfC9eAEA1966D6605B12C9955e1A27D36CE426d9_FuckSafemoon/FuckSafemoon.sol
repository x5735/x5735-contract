...
contract FuckSafemoon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FuckSafemoonDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FuckSafemoon", "$FUCKSFM") {
        marketingWallet = payable(0x64Be618DA250e65e6637174a4d5243a3B1af8078);
        devWallet = payable(0xd553DD6eC0200100E1F435c3F3B0672E72531557);
        ...
        dividendTracker = new FuckSafemoonDividendTracker(
            payable(this),
            router,
...
contract FuckSafemoonDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FuckSafemoon public FuckSafemoonContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FuckSafemoonContract = FuckSafemoon(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FuckSafemoon_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FuckSafemoon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FuckSafemoon contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FuckSafemoonContract);

            uint256 prevBalance = FuckSafemoonContract.balanceOf(address(this));
            ...
            {
                uint256 received = FuckSafemoonContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FuckSafemoonContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
