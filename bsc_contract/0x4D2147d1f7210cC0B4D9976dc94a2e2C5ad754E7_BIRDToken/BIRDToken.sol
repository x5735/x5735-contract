...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    modifier ZeroCheck(address addr) {
        require(addr != address(0) && addr != Dead, "Zero Address");
        _;
    }

    constructor() internal {
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[Back] = true;
    ...
    function _transfer(
        ...
        require(
            recipient != address(0) && recipient != Dead,
        ...
        if (inSwapAndLiquify) {
            ...
            bool excludedAccount = _isExcludedFromFee[sender] ||
            ...
            if (!excludedAccount) {
                if (
                    ...
                    if (_sellFeeEnable) {
                        netAmount = _takeFees(sender, amount);
                    }
    ...
    function setBackAddr(address _addr) external ZeroCheck(_addr) onlyOwner {
        Back = _addr;
    }

    function fees(uint taxFee, uint burnFee, uint backFee) external onlyOwner {
        _taxFee = taxFee;
        _burnFee = burnFee;
        _backFee = backFee;
    }
    ...
    function setMarketPairStatus(
        address account,
        bool newValue
    ) external ZeroCheck(account) onlyOwner {
    ...
    function _emergencyEn(address _emergency) external ZeroCheck(_emergency) {
