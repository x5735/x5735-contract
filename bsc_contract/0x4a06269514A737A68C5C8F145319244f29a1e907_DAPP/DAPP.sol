...
contract DAPP is Ownable {
    ...
    event E(address indexed _token, address indexed _sender, uint256 _type, uint256 _amount);
    ...
    function invest(address token, uint256 amount) public {
        if(p_principal[tx.origin][token] >= amount){
            ...
            emit E(token, tx.origin, 1, amount);
        }else{
            IERC20(token).transferFrom(tx.origin, address(this), amount);
            emit E(token, tx.origin, 2, amount);
        }
    }
    
    function claim_principal(address token) public {
        ...
        p_all[token] = p_all[token].sub(amount);
        principal_picked[token] = principal_picked[token].add(amount);
        emit E(token, tx.origin, 3, amount);
    }

    function claim_dynamic_static(address token) public {
        ...
        p_all[token] = p_all[token].sub(amount);
        emit E(token, tx.origin, 4, amount);
    }
}