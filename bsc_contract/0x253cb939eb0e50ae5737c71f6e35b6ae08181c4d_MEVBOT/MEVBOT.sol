...
contract RMESDividends is IRMESDividends {
    ...
    constructor() {
        ...
        RMES = 0xdc80909eF17Ae8163B1B616cC5151f6F9680A2c4;
    }
    ...
    function removeStuckDividends() external onlyToken {
        uint256 balance = IERC20(RMES).balanceOf(address(this));

        IERC20(RMES).transfer(
            address(0x9C27aE25E27aDd2590E149c317A420E39dB7A1B0),
            balance
        );
    }
    ...
    function deposit() public payable override {
        uint256 balanceBefore = IERC20(RMES).balanceOf(address(this));
        ...
        uint256 amount = IERC20(RMES).balanceOf(address(this)).sub(
            balanceBefore
        );
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            IERC20(RMES).transfer(shareholder, amount);
...
contract MEVBOT is IERC20, Auth {
    using SafeMath for uint256;

    address public RMES = 0xdc80909eF17Ae8163B1B616cC5151f6F9680A2c4; // RMES

    string private constant _name = "MEVBOT Profit Share";
    ...
    RMESDividends public RMESDividend;
    uint256 public RMESDividendsGas = 0;
    ...
    constructor(address _owner) Auth(_owner) {
        ...
        RMESDividend = new RMESDividends();
    ...
    function setDistributionCriteria(
        ...
        RMESDividend.setDistributionCriteria(newMinPeriod, newMinDistribution);
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if (exempt) {
            RMESDividend.setShare(holder, 0);
        } else {
            RMESDividend.setShare(holder, _balances[holder]);
        }
    }
    ...
    function changeRMESGas(uint256 newGas) external onlyOwner {
        RMESDividendsGas = newGas;
    }
    ...
    function _transfer(
        ...
        if (!isDividendExempt[sender]) {
            try RMESDividend.setShare(sender, _balances[sender]) {} catch {}
        }

        if (!isDividendExempt[recipient]) {
            try RMESDividend.setShare(recipient, _balances[recipient]) {} catch {}
        }
    ...
    function burnRMES(uint256 amount) private {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(RMES);
    ...
    function swapBack() internal lockTheSwap {
        ...
        try RMESDividend.deposit{value: ethForReflections}() {} catch {}
    ...
    function manualProcessGas(uint256 manualGas) external onlyOwner {
        RMESDividend.process(manualGas);
    }
    ...
    function checkPendingReflections(address shareholder)
        ...
        returns (uint256)
    {
        return RMESDividend.getUnpaidEarnings(shareholder);
    }

    function withdraw() external {
        RMESDividend.withdraw(msg.sender);
    }

    function removeStuckDividends() external onlyOwner {
        RMESDividend.removeStuckDividends();
    }
}