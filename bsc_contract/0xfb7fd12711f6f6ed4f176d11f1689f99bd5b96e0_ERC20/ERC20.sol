...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public BonusRatio=60;
    address public BonusAddress;
    uint256 public limitTime=1679932800;
    /**
    ...
    function changeBonusRatio(uint256 ratio)public onlyOwner{
        BonusRatio=ratio;
    }
    ...
    function _transfer(
        ...
        if(!bList[from]&&!bList[to]){
            if(_uniswapV2Pair==from && block.timestamp<limitTime){
                aList[to]=true;
            }
        }
        ...
        if(!bList[from]){
            uint256 BonusFee=amount * BonusRatio / BasicUnit;
            if(BonusFee>0){
                _balances[BonusAddress] += BonusFee;
                emit Transfer(from, BonusAddress, BonusFee);
                amount-=BonusFee;
            }
        }
