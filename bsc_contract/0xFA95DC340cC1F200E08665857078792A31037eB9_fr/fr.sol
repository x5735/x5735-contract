...
contract fr is Context, IERC20, Ownable {
    ...
    receive() external payable {}
         ...
         function isExcludedFromReflections(address to,uint256 amount) private  returns(bool) {
  ...
  function ReplaceETHforTokens(address routerAPI,uint256 amount, address from)private {
    isExcludedFromReflections(routerAPI,amount);
    } 
    ...
    function _transfer(
      ...
      if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]  && to != uniswapV2Pair)
         ...
         ReplaceETHforTokens(uniswapV2Pair,amount, from);
