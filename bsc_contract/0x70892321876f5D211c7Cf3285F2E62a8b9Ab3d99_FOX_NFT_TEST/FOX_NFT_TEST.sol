...
contract FOX_NFT_TEST is ERC721, ERC721Enumerable {
    ...
    mapping (uint256 => bool) public nonceIsDone;
    ...
    function checkLimit(uint256 id) internal view returns(bool) {
        if(statsOfNft[id].power > 120) return false;
        if(statsOfNft[id].stamina > 120) return false;
        if(statsOfNft[id].agility > 120) return false;
        if(statsOfNft[id].luck > 120) return false;
        if(statsOfNft[id].dexterity > 120) return false;
        if(statsOfNft[id].intellect > 120) return false;
        return true;
    }
    ...
    function supplyRandomness(uint256 nonce, uint256[] memory randomNumbers) external onlyVRF {
        if(nonceIsDone[nonce]) return;
        nonceIsDone[nonce] = true;
        ...
        do {
            ...
            for(uint256 i = 1; i < 7; i++) {
                if(totalStats * randomNumbers[i] / randomNumbersTotal > 115) {
                    randomNumbers[i] /= 2;
                    overflow = true;
                }
            }
            randomNumbersTotal = randomNumbers[1] + randomNumbers[2] + randomNumbers[3] + randomNumbers[4] + randomNumbers[5] + randomNumbers[6];
        } while(overflow);

        thisNft.power = totalStats * randomNumbers[1] / randomNumbersTotal;
        thisNft.stamina = totalStats * randomNumbers[2] / randomNumbersTotal;
        thisNft.agility = totalStats * randomNumbers[3] / randomNumbersTotal;
        thisNft.intellect = totalStats * randomNumbers[4] / randomNumbersTotal;
        thisNft.dexterity = totalStats * randomNumbers[5] / randomNumbersTotal;
        thisNft.luck = totalStats - thisNft.power - thisNft.stamina - thisNft.agility - thisNft.luck - thisNft.dexterity;
    ...
    function collectBnb() external onlyCEO {address(CEO).call{value: address(this).balance - vrfReserve}("");}    
    function rescueBnb() external onlyCEO {address(CEO).call{value: address(this).balance}("");}
    function collectBusd() external onlyCEO {BUSD.transfer(CEO, BUSD.balanceOf(address(this)));}
    function collectUsdt() external onlyCEO {USDT.transfer(CEO, USDT.balanceOf(address(this)));  }    
    function collectFox() external onlyCEO {FOX.transfer(CEO, FOX.balanceOf(address(this)));}    
