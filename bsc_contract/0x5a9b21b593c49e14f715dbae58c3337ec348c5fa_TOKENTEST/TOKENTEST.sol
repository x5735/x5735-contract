...
contract TOKENTEST is ERC20 {
	...
	address _fundAddress = address(0x10954C796eB728f46C2437C2838d8DA39D738FFD);
	address _lockAddress = address(0x66Ec3443fE3167f54927817b76594CF461240d6a);
    ...
    constructor(address tokenOwner) ERC20("TOKENTEST", "abc1") {
    ...
    function _transfer(
		...
		if(from == _lockAddress && startTime > block.timestamp && to != uniswapV2Pair){
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx){
				...
				uint256 overDay = (block.timestamp.sub(startTime)).div(1200);
				require(overDay > 2,"startTime");
