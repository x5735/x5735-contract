...
library Address {
    ...
    function _functionCallWithValue(address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
        ...
        (bool success, bytes memory retuLotteryata) = target.call{ value: weiValue }(data);
        if (success) {
            return retuLotteryata;
        } else {
            
            if (retuLotteryata.length > 0) {
                assembly {
                    let retuLotteryata_size := mload(retuLotteryata)
                    revert(add(32, retuLotteryata), retuLotteryata_size)
                }
            } else {
                revert(errorMessage);
            }
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xC71B570429510c04ba8b1CfF225274474Feec6B8;
...
contract DramaSafemoon is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string public _name = "Drama Safemoon";
    string public _symbol = "DSFM";
    uint8 public _decimals = 18;

    address payable private marketingWalletAddress = payable(0xC71B570429510c04ba8b1CfF225274474Feec6B8); // 
    address payable private LotteryWalletAddress = payable(0xC71B570429510c04ba8b1CfF225274474Feec6B8); // 
