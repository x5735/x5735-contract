...
contract PinaPine is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "PinaPine";
    ...
    uint256 private totalFee        = devfee + reflectionFee + liquidityFee + ecosystemfee + burnFee;
    ...
    uint256 gweiLimitBlocksAffected = 6;
    ...
    constructor () Auth(msg.sender) {
        ...
        devfeeReceiver = 0x9b4400C54c7d3D6472e65cCAa16128adbC76F0ae;
        ecosystemfeeReceiver = 0x9b4400C54c7d3D6472e65cCAa16128adbC76F0ae;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != devfeeReceiver && recipient != ecosystemfeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _devfee, uint256 _ecosystemfee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        devfee = _devfee;
        ecosystemfee = _ecosystemfee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_devfee).add(_ecosystemfee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _devfeeReceiver, address _ecosystemfeeReceiver, address _burnFeeReceiver ) external authorized {
