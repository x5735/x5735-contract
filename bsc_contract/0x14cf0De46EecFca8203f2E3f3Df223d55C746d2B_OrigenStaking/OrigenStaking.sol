...
contract OrigenStaking is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public devFee = 300; // 3%
    uint256 public ref = 900; // 9%
    uint256 public period = 1 days;
    uint256 public capitalPeriod = 60 * 60 * 24 * 15;
    uint256 public apr = 100; // 1%
    uint256 public percentRate = 10000;

    address private devWallet;
    address public OrigenContract;
    ...
    constructor(address _feesWallet, address _tokenContract) {
        ...
        require(_tokenContract!=address(0),"Please provide a valid Origen contract address");
       
        devWallet = _feesWallet;
        OrigenContract = _tokenContract;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        require(_amount > 0, "you can deposit more than 10 Origen");

        if(_referrer == msg.sender){
            _referrer = address(0);
        }
        IERC20(OrigenContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(OrigenContract).safeTransfer(devWallet,depositFee);
        // transfer 9% fee to referrer
        ...
        if(investors[msg.sender].referrer != address(0)){
            ...
            IERC20(OrigenContract).transfer(investors[msg.sender].referrer, referrerAmount);
        }
    ...
    function claimBonus() public {
        ...
        IERC20(OrigenContract).safeTransfer(msg.sender, _bonus[msg.sender]);
        _bonus[msg.sender] = 0;
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(OrigenContract).safeTransfer(msg.sender,amountToSend);
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(OrigenContract).balanceOf(address(this));
    }
