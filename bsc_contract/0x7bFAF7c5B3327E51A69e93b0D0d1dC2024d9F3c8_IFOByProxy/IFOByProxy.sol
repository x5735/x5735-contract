...
contract IFOByProxy is ReentrancyGuard, Initializable {
  ...
  uint256 public maxCap = 15000000000000000000;    // 15 BUSD
  ...
  // presale
  uint256 public rewardPerBNB = 100; // reward per BUSD
  uint256 public globalDivisor = 1; // 1000000000
  ...
  function modifyGlobalDivisor(uint256 newDivisor) public onlyAdmin {
      globalDivisor = newDivisor;
  }
  

  function deposit(uint256 _amount) public {
    require (block.number > startBlock && block.number < endBlock, 'not ido time');
    require (_amount > 0, 'need _amount higher than 0');
...
function changeWithdraw(bool _bool) public onlyAdmin{
  ...
  function harvest() public nonReentrant {
    ...
    require (canWithdraw == true, "no withdrawal");
    uint256 offeringTokenAmount = userInfo[msg.sender].amount;
    uint256 refundingTokenAmount = userInfo[msg.sender].amount;
    if(totalAmount >= raisingAmount ){
        offeringToken.safeTransfer(address(msg.sender), offeringTokenAmount.mul(rewardPerBNB).div(globalDivisor));
    }
    else {
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    ...
    //   uint256 allocation = getUserAllocation(_user);
      return userInfo[msg.sender].amount.mul(rewardPerBNB).div(globalDivisor);
    }
    else {
  ...
  function burnTokens() public onlyAdmin{
    ...
    if(offeringToken.balanceOf(address(this)) > 0) {
      offeringToken.safeTransfer(msg.sender, totalToBurn.mul(rewardPerBNB).div(1000000000));
    }
