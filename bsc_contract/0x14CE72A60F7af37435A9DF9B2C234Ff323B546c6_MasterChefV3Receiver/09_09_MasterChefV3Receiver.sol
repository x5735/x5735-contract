...
contract MasterChefV3Receiver is Ownable {
    ...
    event UpdateMCV3(address indexed newMCV3);
    ...
    /// @notice Set MasterChefV3 address.
    /// @dev In case MasterChefV3 change , because MasterChefV3 address has not been determined at the time of deployment.
    /// @dev Callable by owner.
    /// @param _newMCV3 New MCV3 address.
    function setMCV3(IMasterChefV3 _newMCV3) external onlyOwner {
        if (address(_newMCV3) == address(0)) revert ZeroAddress();
        MasterChefV3 = _newMCV3;
        emit UpdateMCV3(address(_newMCV3));
    }
