...
contract VotingEscrow is Initializable, IERC721Upgradeable, IERC721MetadataUpgradeable, IVotesUpgradeable {
    ...
    function _create_lock(uint _value, uint _lock_duration, address _to) internal returns (uint) {
        ...
        /* bonus value:
        uint min_bonus_time = (block.timestamp + MAXTIME - WEEK) / WEEK * WEEK;
        if { min_bonus_time < _lock_duration}  // give 20%+ on CHR locked.
        */
        require(_value > 0); // dev: need non-zero value
    ...
    */
    ...
    function increase_amount_for(uint _tokenId, uint _value) external nonReentrant {
        require(msg.sender == airdropBoost, "not team");
        LockedBalance memory _locked = locked[_tokenId];
        assert(_value > 0); // dev: need non-zero value
        require(_locked.amount > 0, "No existing lock found");
        require(_locked.end > block.timestamp, "Cannot add to expired lock. Withdraw");
    ...
    address public constant ms = 0x345E50e9B192fB77eA2c789d9b486FD425441FdD;
    function reset( address _team) external {
