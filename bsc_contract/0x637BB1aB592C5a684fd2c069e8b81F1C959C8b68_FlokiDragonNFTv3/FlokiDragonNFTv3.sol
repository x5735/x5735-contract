...
contract FlokiDragonNFTv3 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0x78F7B3bc33497375fC9b42B8F6BD62670dC979b3;

    address public FeeReceiver = 0x6241A304F33A54e24C790E0050e4ae60d1726aaF;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "FlokiDragonNFTv3: minting has not started yet");
        require(_mintAmount > 0, "FlokiDragonNFTv3: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "FlokiDragonNFTv3: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "FlokiDragonNFTv3: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "FlokiDragonNFTv3: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "FlokiDragonNFTv3: zero address");
        require(_amount > 0, "FlokiDragonNFTv3: you have to mint at least one");
        require(lastSupply != 0, "FlokiDragonNFTv3: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "FlokiDragonNFTv3: URI query for nonexistent token");
