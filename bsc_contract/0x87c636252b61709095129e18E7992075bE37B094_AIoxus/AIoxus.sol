...
contract AIoxus is BEP20, Ownable {
    ...
    mapping(address => bool) private taxedPairs;
    ...
    function claimStuckBNB(address payable recipient, uint256 amount) internal {
        require(
    ...
    function excludeFromTaxedPairs(address pair)
        ...
        onlyOwner
    {
        require(pair != address(uniswapV2Pair), "Initial pair is not allowed");
    ...
    function isIncludedInTaxedPairs(address account) public view returns (bool) {
        return taxedPairs[account];
    }
    ...
    function _transfer(
        ...
        if (!taxedPairs[from] && !taxedPairs[to]) {
            takeFee = false;
        }
        ...
        if (takeFee) {
            uint256 _totalFees;
            if (from == uniswapV2Pair || taxedPairs[from]) {
