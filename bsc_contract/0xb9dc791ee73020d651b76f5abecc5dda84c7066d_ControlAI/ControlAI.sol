...
Control2XY is an AI Token.
One of the key features of Control2XY.ai is its use of AI technology. The token is designed to be highly responsive to market conditions and to adjust its deflationary rate accordingly. This ensures that the token remains stable and retains its value over the long term.
...
contract ControlAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ControlAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ControlAI", "CtrlAI") {
        marketingWallet = payable(0x5bA3FdfF8f52e7c712F8E21f6F333B9Ab98c57C6);
        devWallet = payable(0xAfeDF689CA457439Cc4344a20a17931b1C7585f5);
        ...
        dividendTracker = new ControlAIDividendTracker(
            payable(this),
            router,
...
contract ControlAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ControlAI public ControlAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ControlAIContract = ControlAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ControlAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ControlAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ControlAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ControlAIContract);

            uint256 prevBalance = ControlAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = ControlAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ControlAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
