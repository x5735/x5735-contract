...
/*
Maturation Days ¨C the number of days that must elapse after a deposit before an investor can unstake without penalty (e.g., 90 days) 
Starting Burn Rate ¨C the initial burn rate, which will decrease over time until Maturation (defaults to 20%) 
Reward Rate ¨C a multiplier applied to the staking amount, used to calculate the reward tokens due to an investor at Maturation 
*/
