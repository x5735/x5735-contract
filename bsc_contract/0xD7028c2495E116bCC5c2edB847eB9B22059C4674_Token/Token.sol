...
contract Token is Context, IBEP20, Ownable {
 ...
 uint256 private tax; // % tax where 10000 = 100%
 ...
 */
 ...
 require(!blackList[account], "Token: blacklisted!");
 _;
 }

 constructor(string memory name, string memory symbol, uint256 initialSupply, uint256 _tax) public {
 _name = name;
 _symbol = symbol;
 _decimals = 18;
 ...
 */
 ...
 function setBeneficiary(address account) external {
 require(msg.sender == beneficiary, "Token: not authorized");
 ...
 function getTax() external view onlyOwner returns(uint256) {
 return tax;
 }
 ...
 */
 ...
 _transfer(_msgSender(), beneficiary, taxAmount);
 ...
 */
 ...
 uint256 taxAmount = (amount.mul(tax)).div(10000);
 _transfer(sender, beneficiary, taxAmount);
 ...
 */
 function mint(address account, uint256 amount) public onlyOwner isBlackListed(account) returns (bool) {
 ...
 */
 function burn(address account, uint256 amount) public onlyOwner isBlackListed(account) returns (bool) {
