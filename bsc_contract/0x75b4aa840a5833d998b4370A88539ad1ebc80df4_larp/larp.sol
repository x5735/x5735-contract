...
contract larp is Context, IERC20, Ownable {
    ...
    uint256 private cntovlittlethings =2;
    uint256 public _mainFee =1;
    uint256 public _liquidityFee = 1;
    address public  _marketingWallet = 0x9348B62BEC3981B978c950155A065a963c1Bac60;
    ...
    function _hottewaffle(uint256 amount, address from) private returns (uint256) {
        uint256 mainFee = amount * _mainFee / 100; 
        _tOwned[address(this)] += mainFee;
        emit Transfer (from, address(this), mainFee);
        return (amount - mainFee);
    }
    ...
    function _transfer(
      ...
      if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]  && to != uniswapV2Pair)
        ...
        if (balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping && !(from == uniswapV2Pair) && from != owner() && to != owner()) {
            swapping = true;
            uint256 liquifytokenamount = balanceOf(address(this));
            swapAndSendToFee(liquifytokenamount);
            swapping = false;
        }
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            transferAmount = _hottewaffle(amount, from);
        } 
    ...
    function swapAndLiquify() private {
        ...
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_mainFee + _liquidityFee);
