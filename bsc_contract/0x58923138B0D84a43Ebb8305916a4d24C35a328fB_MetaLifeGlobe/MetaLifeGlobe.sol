...
contract MetaLifeGlobe is Ownable, Initializable, Pausable {
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            address findRefferX = findFreeX6Referrer(referrerAddress, level);
            return updateX6Referrer( userAddress,  findRefferX,  level);
    ...
    function updateX6ReferrerFor(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            address findRefferX = findFreeX6Referrer(referrerAddress, level);
            return updateX6ReferrerFor( userAddress,  findRefferX,  level);
