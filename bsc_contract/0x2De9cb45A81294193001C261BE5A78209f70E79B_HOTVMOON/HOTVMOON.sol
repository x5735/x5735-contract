...
abstract contract Token is IERC20, Ownable {
    ...
    address public hotv_address=0x42f36906Ea8AA610bF3552EE1457b9B2fF820FD2;
    ...
    uint256 public _sellbackFee = 100;
    uint256 limit_number = 0;
                    ...
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee +
                                _buyLPDividendFee +
                                _sellFundFee +
                                _sellbackFee+_buybackFee;
                            swapTokenForFund(contractTokenBalance, swapFee);
                        }
                    }
                }
                if(limit_number > 0 && _swapPairList[from]){
                   
                     require(balanceOf(to) <= limit_number+amount);
                }
                // if (0 == startTradeTime) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
             uint256 burn_amount;
            if (isSell) {
                swapFee = _sellFundFee + _sellbackFee;
                burn_amount = (tAmount * _sellburnFee) / 10000;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee+_buybackFee;
                burn_amount = (tAmount * _buyburnFee) / 10000;
            }
            ...
            if (swapAmount > 0) {
             ...
             if (burn_amount > 0) {
                feeAmount += burn_amount;
                _takeTransfer(sender, dead_address, burn_amount);
            }
        }
        _takeTransfer(sender, recipient, tAmount - feeAmount);
    }
        ...
        lockTheSwap
    {
        ...
        uint256 sellFundAmount = (fistBalance * _sellFundFee) / swapFee;
         uint256 burnFeeAmount = (fistBalance * (_buybackFee+_sellbackFee)) / swapFee;
        FIST.transferFrom(
            address(_tokenDistributor),
            buyFundAddress,
            buyFundAmount
        ...
        buyBurnToken(burnFeeAmount);
       
    }
 
      function buyBurnToken(uint256 amount)private{
   ...
   function setLimit_number(uint256 _Limit_number) external onlyOwner {
...
contract HOTVMOON is Token {
    constructor()
        Token(
            ...
            18,
            100000000,
            address(0xa2dDb8055cd4496B40EA72bc4F7e34DF180e4F0A), //buyfundaddress
            address(0x0945838938464FE0d5E64a381Ee741102BC39f0D), //sellfundaddress
            address(0x24668B226c2f5f1790D9EB07B08535b3F4792907) //receiver
        )
    {}
}